...
-- SIAC-7676 - Maurizio - INIZIO

update siac_t_report
set rep_ordina_elenco_variab= rep_ordina_elenco_variab+1
where rep_ordina_elenco_variab > 26;

insert into siac_t_report(
rep_codice ,  rep_desc,  rep_birt_codice,  validita_inizio,
  ente_proprietario_id ,  data_creazione ,  data_modifica ,
  login_operazione,  rep_ordina_elenco_variab)
select 'BILR203',  
  'Rendiconto - Allegato al Rendiconto - Entrate: Riepilogo per Titolo - Tipologia - Capitolo (BILR203)',
  'BILR203_Conto_gestione_entrate_tit_tipol_capit', now(),
  ...
  where ente.data_cancellazione IS NULL
  	and not exists (select 1
        ...
        where t_rep.rep_codice='BILR203'
        	and t_rep.data_cancellazione IS NULL
            and t_rep.ente_proprietario_id=ente.ente_proprietario_id) ;
  
 insert into siac_r_report_importi (
  rep_id,  repimp_id,  posizione_stampa,  validita_inizio,
  ente_proprietario_id ,  data_creazione ,  data_modifica ,
  login_operazione)
 select (select rep_id
 		from siac_t_report t_rep
        where t_rep.rep_codice='BILR203'
        	and t_rep.data_cancellazione IS NULL
            and t_rep.ente_proprietario_id=ente.ente_proprietario_id),
         (select b.repimp_id
          ...
          where a.rep_id=c.rep_id
            ...
            and a.rep_codice='BILR048'
            and b.repimp_codice='di_cui_ant_liq_rend'
            and e.anno='2019'
            ...
            and e.data_cancellazione IS NULL), 1, now(),
            ente.ente_proprietario_id, now(), now(),'SIAC-7676'                        
 from siac_t_ente_proprietario ente
 where ente.data_cancellazione IS NULL
	and ente.ente_proprietario_id not IN(7,8);
...
-- SIAC-7676 - Maurizio - FINE

-- SIAC-7684 - 23.06.2020 Sofia inizio 

drop VIEW if exists siac_v_dwh_variazione_bilancio;
...
-- SIAC-7684 - 23.06.2020 Sofia fine
...
BEGIN
	...
	-- In entrambe c'  sia il bil_id che il bil_id_orig ovvero l'anno nuovo su cui si stanno ribaltando i movimenti e l'anno prec
	-- c'  anche un campo fl_elab che ti dice se i dati sono stati elaborati con scarto o con successo
	...
	if NOT FOUND THEN
		...
		if esisteRmovgestidelemid <>0 then
			...
			-- if importoCurAttuale>0 then
			 ...
			 Non   pi  richiesto di restituire all'impegnato le modifiche provvisorie 
				if importoCurAttuale>=0 then -- 05.09.2017 Sofia siac-5198 il conteggio deve essere effettuato anche se impoatt=0
    ...
    importoModifDelta:=0;
    --END FIX
	impegnatoDefinitivo:=0; -- SIAC-7349 evito di tornare null nei casi in cui non c'  alcun impegnato sulla componente per l'anno N-1
...
CASE loop_counter
 ...
 WHEN 0 THEN
	...
	-- Non dobbiamo pi  restituire al impegnato le modifiche provvisorie e le ECONB
	importoModifDelta:=0;
	importoModifINS:=0;
	--
 ...
 WHEN 5 THEN
	...
	annoEsercizio:=annoBilancio;
	annoMovimento:=annoBilancio;
	flagDeltaPagamenti:=0; -- non   necessario scomputare il pagato
...
--Start fnc_siac_dicuiimpegnatoug_anniprec
...
--end fnc_siac_dicuiimpegnatoug_anniprec
 ...
 * esistono quote di impegnato nell'anno > N+2 
 * Le componenti per le quali esiste stanziamento (e quindi da non considerare) sono passate in input nell'array idcomp_in
 * NOTA: per problemi Java/Hibernate idcomp_in non pu  essere un array vuoto, quindi 
 *		l'array in input conterr  sempre la componente fittizia idcomp = -1
 */
-- CALCOLO IMPEGNATO DEFINITIVO 
-- I valori di impegnato dovranno essere calcolati tenendo conto del solo impegnato definitivo.
-- Escluse dal calcolo quindi:
...
-- 		- gli  impegni in stato provvisorio
	...
	strMessaggio:='Calcolo totale impegnato definitvo elem_id='||id_in||' escludo idcomp_in='||sidcomp_in||
 	...
 	-- Calcolo Impegnato definitivo   Anno > N+2:
	-- o Se presenti i movimenti gestione provenienti dal ribaltamento 
	--		Sommatoria di tutti gli Impegni assunti sul capitolo in questione su Componente e anno movimento > N+2 e anno esercizio N
	-- o Se non presenti i movimenti gestione provenienti dal ribaltamento 
...
*/
			...
			-- Non restituiamo pi  all'impegnato il valore delle modifiche negative provvisorie
			...
			elemdetcompdesc:=impComponenteRec.compDesc;
			elemdetcompmacrotipodesc:=impComponenteRec.compMacroTipoDesc;
			impegnatoDefinitivo:=0; -- SIAC-7349 evito di tornare null nei casi in cui non c'  alcun impegnato sulla componente  
			impegnatoDefinitivo:=impegnatoDefinitivo+impComponenteRec.importoCurAttuale-(importoModifDelta);
			--aggiunta per ECONB
			impegnatoDefinitivo:=impegnatoDefinitivo+abs(importoModifINS);
			return next;	
...
CASE loop_counter
 ...
 WHEN 0 THEN
	...
	-- Sommatoria di tutti gli Impegni assunti sul capitolo di previsione id_in su Componente idcomp_in
	-- con anno movimento N-1 e anno esercizio N-1.
	-- Il capitolo usato per determinare gli impegni   quello UG Equivalente gi  esistente nel bilancio dell esercizio N-1
	-- I valori di impegnato dovranno essere calcolati tenendo conto del solo impegnato definitivo.
	-- Escluse dal calcolo quindi:
	--		- le modifiche di impegno provvisorie (modifiche con provvedimento in stato provvisorio - di qualsiasi segno)
	-- 		- gli  impegni in stato provvisorio
