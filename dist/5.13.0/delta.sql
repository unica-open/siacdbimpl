...
ALTER TABLE siac.siac_t_ente_proprietario ADD if not exists ente_code varchar NULL;

update siac.siac_t_ente_proprietario set ente_code = 'REGP' where codice_fiscale in ('80087670016', '1c890f326e966fab');
...
(values (
--
'allegatoAtto.invia.abilitaChecklist', 'Abilitazione checklist per invia allegato atto'
--
)) as x (nome, note) 
...
CREATE TABLE IF NOT EXISTS siac.siac_d_atto_allegato_checklist (
	atto_allegato_checklist_id SERIAL,
	atto_allegato_checklist_code varchar(250),
	atto_allegato_checklist_desc varchar(250),
	...
	CONSTRAINT pk_siac_d_atto_allegato_checklist PRIMARY KEY (atto_allegato_checklist_id),
	CONSTRAINT siac_t_ente_proprietario_siac_d_atto_allegato_checklist FOREIGN KEY (ente_proprietario_id) REFERENCES siac.siac_t_ente_proprietario(ente_proprietario_id)
);


CREATE UNIQUE INDEX IF NOT EXISTS  siac_d_atto_allegato_checklist_ente_proprietario_id_idx 
ON siac.siac_d_atto_allegato_checklist (ente_proprietario_id,atto_allegato_checklist_code,data_cancellazione);





insert into siac_d_atto_allegato_checklist (
	ente_proprietario_id,
	atto_allegato_checklist_code,
	atto_allegato_checklist_desc,
	login_operazione 
) select 
	e.ente_proprietario_id ,
	x.code,
...
(values 
--
( 'soggettoCreditore', 'Individuazione del soggetto creditore, fiscalmente identificato' ),
( 'sommaDovuta', 'Somma dovuta' ),
( 'causale', 'Causale' ),
( 'modalitaPagamento', 'Modalita Pagamento' ),
( 'scadenza', 'Scadenza' ),
( 'estremiProvvedimentoDirigenziale', 'Estremi Provvedimento Dirigenziale' ),
( 'allegati', 'Allegati' ),
( 'esigibileSpesa', 'Esigibile Spesa' ),
( 'congruitaSpesaSommaImpegnata', 'Congruita Spesa Somma Impegnata' ),
( 'applicazioneNormativaFiscale', 'Applicazione Normativa Fiscale' ),
( 'iva', 'IVA' ),
( 'entrataVincolata', 'Entrata Vincolata' ),
...
( 'regolaritaContabile', 'Regolarita Contabile' )
--
) as x (code, descr) 
where not exists ( select 1 from 
	siac_d_atto_allegato_checklist p 
	where atto_allegato_checklist_code = x.code 
	and e.ente_proprietario_id = p.ente_proprietario_id)
...
--- SIAC-8823 07.11.2022 Sofia - inizio 
...
--- SIAC-8823 07.11.2022 Sofia - fine 


--SIAC-8817 - Maurizio - INIZIO
...
--SIAC-8817 - Maurizio - FINE

-- SIAC-8846 - Sofia - inizio 
...
-- SIAC-8846 - Sofia - fine
-- 18.11.2022 - SIAC-8855 Sofia - inizio
...
drop FUNCTION if exists siac.fnc_siac_dicuiimpegnatoug_econb_anno2
(
  id_in integer
);

drop FUNCTION if exists siac.fnc_siac_dicuiimpegnatoug_econb_anno3
(
  id_in integer
);
...
CREATE OR REPLACE FUNCTION siac.fnc_siac_dicuiimpegnatoug_econb_anno2
(
  id_in integer
)
RETURNS numeric AS
...
BEGIN
    ...
    annoCompetenza:=annoBilancio::integer+1;

    strMessaggio:='Calcolo totale movimenti modifiche negative prov e econb elem_id='||id_in||
                             '.Calcolo per anno='||annoCompetenza::varchar||'.';

    select * into resultRec
    from fnc_siac_dicuiimpegnatoug_econb_anno  (id_in,annoCompetenza::varchar);
    totImportoEconB:=resultRec.dicuiimpegnato_econb;
	if totImportoEconB is null then totImportoEconB:=0; end if;
...
alter function siac.fnc_siac_dicuiimpegnatoug_econb_anno2( integer ) owner to siac;

CREATE OR REPLACE FUNCTION siac.fnc_siac_dicuiimpegnatoug_econb_anno3
(
  id_in integer
)
RETURNS numeric AS
...
BEGIN
    ...
    annoCompetenza:=annoBilancio::integer+2;
...
alter function siac.fnc_siac_dicuiimpegnatoug_econb_anno3( integer) owner to siac;

-- 18.11.2022 - SIAC-8855 Sofia - fine 
...
(values (
--
'impegno.obbligo.perimetroSanitario', 'Obbligatorieta'' perimetro sanitario dell''impegno'
--
)) as x (nome, note) 
where not exists ( select 1 from siac_t_parametro_config_ente p where parametro_nome = x.nome and e.ente_proprietario_id = p.ente_proprietario_id); 
...
-- 8873
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='Somma dovuta e causale' where atto_allegato_checklist_code ='sommaDovuta';
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='Allegati' where atto_allegato_checklist_code ='allegati';
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='Modalità pagamento' where atto_allegato_checklist_code ='modalitaPagamento';
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='Spesa certa liquida ed esigibile' where atto_allegato_checklist_code ='esigibileSpesa';
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='Congruita Spesa con Somma Impegnata' where atto_allegato_checklist_code ='congruitaSpesaSommaImpegnata';
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='Applicazione Normativa Fiscale' where atto_allegato_checklist_code ='applicazioneNormativaFiscale';
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='IVA' where atto_allegato_checklist_code ='iva';
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='Entrata Vincolata' where atto_allegato_checklist_code ='entrataVincolata';
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='Accertamento' where atto_allegato_checklist_code ='accertamento';
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='Incasso' where atto_allegato_checklist_code ='incasso';
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='Correttezza CIG' where atto_allegato_checklist_code ='cig';
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='Correttezza CUP' where atto_allegato_checklist_code ='cup';
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='Contributiva (esito DURC ok)' where atto_allegato_checklist_code ='contributiva';
update siac_d_atto_allegato_checklist set atto_allegato_checklist_desc ='Adempimento Pubblicazione' where atto_allegato_checklist_code ='pubblicazione';
-- 8873
