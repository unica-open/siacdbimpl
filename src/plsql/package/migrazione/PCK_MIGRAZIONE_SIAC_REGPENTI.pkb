...
-- DAVIDE : CONTROLLO IMPEGNI / ACCERTAMENTI 
--          SCARTO DEI RECORD CON NUMERO PROVVEDIMENTO NON NUMERICO    
...
begin
    ...
    (select migr_capusc_id_seq.nextval,'CAP-UP',cAnno.anno_esercizio,
            cAnno.nro_capitolo,cAnno.nro_articolo,1,cAnno.descri,null,
            tit118.titolo, 
            decode(nvl(macr118.macroaggreg,' '),' ' ,null,macr118.titolo||macr118.macroaggreg||'0000'),
            miss118.missione,
            decode(nvl(progr118.programma, ' '),' ',null,progr118.missione||progr118.programma),
	...
	-- DAVIDE - 12.11.015 - Fine		  
            ...
            null,null,
            decode(nvl(cdr.centro_resp,' '),' ','000',cdr.direzione), null,
            --nvl(cdr.centro_resp,'00'), 26.10.2015 Sofia
    ...
    --            0,cAnno2.st_prev,0,cAnno3.st_prev,p_ente
     ...
     from previsione_uscita cAnno, previsione_uscita cAnno2,previsione_uscita cAnno3, d118_prev_usc cap118,
          ...
          centri_resp cdr,migr_capitolo_eccezione capEcc	
		  -- DAVIDE - 04.11.015 aggiunti campi da migrare "di cui impegnato"
     ...
     where cAnno.anno_creazione=p_anno_esercizio and
           ...
           cdr.centro_resp (+) =cAnno.Centro_Resp and
...
begin
    ...
    (select migr_capent_id_seq.nextval,
            ...
            tit118.titolo,
            decode(nvl(tip118.tipologia,' '), ' ' ,null,tip118.titolo||'0'||tip118.tipologia||'00'),cat118.categoria,
            decode(nvl(cap118.conto,' '),' ', null,decode(pdcFin.Livello,4,pdcFin.Conto,null)),
            decode(nvl(cap118.conto,' '), ' ',null, decode(pdcFin.Livello,5,pdcFin.Conto,null)),
            cAnno.Note,cAnno.Permem_Prev, cAnno.Rilev_Iva,
            decode(nvl(tipoFin.Tipofin,' '),' ',null, tipoFin.Tipofin||'||'||tipoFin.Descri ),
...
--                migrazione_accertamenti(p_ente_proprietario_id, v_anno, v_codRes, v_imp_inseriti, v_imp_scartati,p_msgRes);
    ...
    is
       ...
       segnalare integer := 0; -- True (1): il record � inserito in entrambe le tabelle migr_* e migr_*_scarto
                                   -- Flase (0): il record � inserito nella sola tabella migr_*
begin
       p_imp_scartati:=0;
       p_imp_inseriti:=0;
       p_cod_res:=0;
	   ...
	   ( select i.anno_esercizio,i.annoimp anno_impegno,i.nimp numero_impegno,i.nsubimp numero_subimpegno,
                ...
                decode(nvl(i.nprov,'X'),'X',null,i.nprov) numero_provvedimento,
       ...
       loop
			   ...
			   -- DAVIDE : CONTROLLO IMPEGNI / ACCERTAMENTI - SCARTO DEI RECORD CON NUMERO PROVVEDIMENTO NON NUMERICO                   
               if IsNumeric(migrImpegno.numero_provvedimento) <> 0 then
                  msgRes:='Numero Provvedimento non numerico -> '||migrImpegno.numero_provvedimento||'.';
                  codRes:=-1;
               else
                   if codRes=0 then
  ...
  --                          codRes:=-1;
                   ...
                   if codRes=0 then
                     ...
                     if migrImpegno.numero_provvedimento is null or migrImpegno.numero_provvedimento ='0' then
                       ...
                       h_numero_provvedimento:=to_number(migrImpegno.numero_provvedimento);
                       ...
                       -- 29.7.2015 Dani Lo stato del subimp con provvedimento � DEFINITO.
  ...
  when others then
    ...
    procedure migrazione_accertamenti(p_ente_proprietario_id number,p_anno_bilancio varchar2,p_anno_esercizio varchar2,p_cod_res out number, p_imp_inseriti out number, p_imp_scartati out number,pMsgRes out varchar2)
     is
       ...
       segnalare integer := 0; -- True(1): il record � inserito in entrambe le tabelle migr_* e migr_*_scarto
                                   -- False(0): il record � inserito nella sola tabella migr_*
begin
       p_imp_scartati:=0;
       p_imp_inseriti:=0;
       p_cod_res:=0;
...
--                          codRes:=-1;
                 ...
                 if codRes=0 then
                   ...
                   if migrImpegno.numero_provvedimento is null or migrImpegno.numero_provvedimento ='0' then
                    ...
                    -- 29.7.2015 Dani Lo stato del subacc con provvedimento � DEFINITO.
                     h_stato_impegno := STATO_D;
                     if codRes=0 and h_stato_provvedimento is null then
                       h_stato_provvedimento:=h_stato_impegno;
                     end if;
        ...
        exception when others then
               ...
               for migrCursor in (
                 ...
                 loop
                        ...
                        -- DAVIDE : CONTROLLO LIQUIDAZIONI - SCARTO DEI RECORD CON NUMERO PROVVEDIMENTO NON NUMERICO
                        if IsNumeric(migrCursor.nprov) <> 0 then
                            msgRes:='Numero Provvedimento non numerico';
                            codRes:=-1;
                        else
...
/*                                     -- 18.09.2015
  ...
  -- 02.11.2015 Replica migrazione documenti , da pck di regione
...
--      aaL.Anno_Provvedimento_aa||'\'||aaL.Numero_Provvedimento_aa||'\'||aaL.Tipo_Provvedimento_aa||'\'||aaL.Sac_Provvedimento_aa||'\'||el.Numero_Elenco||'\1' numero
     ...
     insert into migr_docquo_spesa
       (docquospesa_id,
        ...
        --tipo_avviso, NULL non gestito
        flag_esproprio, -- valore di default N
        flag_manuale, -- valore di default N
        note,
        causale_ordinativo,
        numero_mutuo, -- valore di default 0
        --annotazione_certif_crediti,
