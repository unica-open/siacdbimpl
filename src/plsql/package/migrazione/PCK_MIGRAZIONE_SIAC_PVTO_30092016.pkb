...
begin
    ...
    delete migr_capitolo_uscita
     ...
     -- pulizia tabella migr_capitolo_eccezione Anna V. 04/07/2016
      ...
      select distinct 'P','U',p_anno_esercizio,cAnno.Nro_Capitolo_77,0,'1','N','FPV',3
       from prev_peg cAnno
       where cAnno.anno_ese=p_anno_esercizio-1
       and   cAnno.Tipo_Int_77='U'
       and   cAnno.Tipo_Cap_Gemello='FPV'
      union
      select distinct 'P','U',p_anno_esercizio,cAnno.Nro_Capitolo_77,0,'1','N','FSC',3
       from prev_peg cAnno
       where cAnno.anno_ese=p_anno_esercizio-1
       and   cAnno.Tipo_Int_77='U'
  ...
  commit;
  /* 26.11.2015 Sofia commentata perche non esiste pi� la tabella
...
con res.anno_peg=m.anno_esercizio*/
     ...
     update migr_capitolo_uscita  m
         --ANNA V 30/06/2016 commentato perch� in fase di caricamento lo stanziamento_iniziale_cassa � uguale allo stanziamento_cassa
     set --m.stanziamento_iniziale_cassa=m.stanziamento_iniziale_cassa+m.stanziamento_iniziale_res,
         ...
         -- ANNA V. commentato perch� � stato inserito l'importo cassa in preventivo
...
--la transazione europea Anna V
--le funzioni delegate Anna V
...
/*  26.11.2015 Sofia commentata perch� non esiste pi� la tabella
...
con res.anno_peg=m.anno_esercizio*/
     ...
     where m.anno_esercizio=p_anno_esercizio and m.tipo_capitolo='CAP-EP' and
      ...
      update migr_capitolo_entrata  m
      set -- ANNA V. commentato perch� � in fase di caricamento preventivo stanziamento_iniziale_cassa=stanziamento_cassa
...
procedure reset_seq(p_seq_name in varchar2) is
  ...
  -- DAVIDE - 16.12.015 - Fine
     ...
     and m.ente_proprietario_id=p_ente_proprietario_id;
      ...
      -- 09.10.2015 se il soggetto associato � un ati, viene trattato come soggetto non determinato
...
/*                        where mc.tipo_cap=p_tipo_cap 22.09.015 Sofia - modifica dopo incontro con Valenzano
                        ...
                        and mc.anno_delibera = a.anno_prot
                        and mc.nro_delibera = a.nro_prot
                        and mc.cod_fornitore IS NOT NULL and mc.cod_fornitore <> COD_SOGGETTO_NULL -- condizione da eliminare perch� tutti i sub devono avere il soggetto determinato, necessaria bonifica dei dati
                        order by 2,3,4) loop*/
      ...
      -- pdc_finanziario � recuperato dall'impegno padre
      if codRes = 0 then
        if p_tipo_cap = TIPO_CAP_USCITA then
...
--            migrazione_accertamento(p_ente_proprietario_id, p_anno_esercizio, v_codRes, v_imp_inseriti, v_imp_scartati,p_msgRes);
  ...
  -- DAVIDE - 16.12.015 - Fine
                ...
                and nvl(asm.residuo_da_riportare,0)>0 -- disponibilit� della liquidazione
...
--                                     msgRes := 'Soggetto determinato non migrato.';
    ...
    /* la verifica del soggetto migrato � fatto nel loop
 ...
 -- liquidazioni legate ad impegni
  ...
  procedure migrazione_doc_spesa (pEnte number,pLoginOperazione varchar2,pCodRes out number,pRecInseriti out number,pRecScartati out number,pMsgRes out varchar2)
   is
        ...
        h_codice_soggetto number:=null; -- coincide con il codice soggetto del doc se questo non � una sede secondaria, altrimenti � il codice soggetto di riferimento della sede secondaria

        h_sede_id number:=null; --Valorizzata con il campo migr_sede_secodiaria.sede_id se il soggetto legato al doc � una sede secondaria
  ...
  -- DAVIDE - 10.02.016 - Gestire nuovo campo anno_repertorio
      ...
      --, data_sospensione,    -- per il momento non valorizzata
          ...
          codRes := -1;
            ...
            if codRes = 0 then
                -- scartiamo i doc per cui non � stato possibile definirne il tipo, il dato deve essere bonificato.
  ...
  PE-->  importo_liquidato>importo_pagato*/
            ...
            if codRes = 0 then
              ...
              (docspesa_id,
               ...
               anno,
               numero,
               codice_soggetto_fonte, -- � il dato letto dalla vista
               codice_soggetto, -- coincide con il dato letto dalla vista se il soggetto non � sede secondaria, in alternativa � il codice del soggetto di riferimento della sede
               sede_id, -- � l'id della tabella migr_sede_secondaria
...
--               anno_registro_fatt, non � letto dalla vista
              ...
              (migr_doc_spesa_id_seq.nextval,
               ...
               h_codice_soggetto,
               h_sede_id, --� valorizzato se il soggetto del doc � una sede secondaria
               0,-- codben_pagamento
               h_stato,
               migrCursor.descrizione,
               migrCursor.data_emissione,
   ...
   begin
    ...
    -- DAVIDE - 10.02.016 - inserito il valore di default in commissioni
        ...
        loop
          ...
          h_progben := NULL; -- modalit� di pagamento valorizzata solo se quota non pagata
          ...
          if codRes = 0 then
              ...
              then
              -- 1 Verifica Modalit� di pagamento se valorizzata
              ...
              if h_movimentoMigrato = 0 then
                 ...
                 exception
                  ...
                  when too_many_rows then
                    h_motivoScarto:='Movimento scartato per pi� di un motivo.'; -- migliorare concatenando i motivi di scarto del movimento.
                  when others then
                      msgRes := msgRes || ' Errore ' || SQLCODE || '-' ||SUBSTR(SQLERRM, 1, 100);
                      RAISE ERROR_DOCUMENTO;
                 end;
                 ...
                 begin
                  ...
                  exception
                    when no_data_found then
                      h_motivoScarto:=NULL;
                    when too_many_rows then
                      h_motivoScarto:='Liquidazione scartata per pi� di un motivo.'; -- migliorare concatenando i motivi di scarto della liquidazione.
                    when others then
                        msgRes := msgRes || ' Errore ' || SQLCODE || '-' ||SUBSTR(SQLERRM, 1, 100);
                        RAISE ERROR_DOCUMENTO;
                  end;
...
--            end;-- chiude blocco begin
            ...
            msgRes := 'Quota Pagata.';
            -- la quota � pagata, integriamo le note
            if migrCursor.numero_subimpegno = 0 then
