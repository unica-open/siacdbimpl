...
begin
    ...
    insert into migr_capitolo_uscita
      (capusc_id,tipo_capitolo,anno_esercizio,numero_capitolo,numero_articolo,numero_ueb,
	   ...
	   spesa_ricorrente)                                                 -- DAVIDE - 23.08.2016 - aggiunto campo per Capitoli Uscita
      (
...
--17.07.2015 Lusso: Per poter stampare correttamente la colonna 'Previsioni definitive anno precedente' dei report ufficiali, � necessario impostare lo stanziamento inziale
...
-- 21.04.2016 Sofia             cUebAnno.St_Anno_Prec,0,0,
              cUebAnno.St_Anno_Prec,nvl(cUebAnno.st_residui,0),nvl(cUebAnno.st_cassa,0),              
-- 21.04.2016 Sofia             cUebAnno.St_Prev,0,0,
              cUebAnno.St_Prev,nvl(cUebAnno.st_residui,0),nvl(cUebAnno.st_cassa,0),              
...
--              cUebAnno3.St_Anno_Prec,cUebAnno3.st_prev, p_ente
              ...
              decode(nvl(cAnno.tipo_spesa,''), 'F' ,'3', 'U', '4', cAnno.tipo_spesa)                          -- DAVIDE - 23.08.2016 - aggiunto campo per Capitoli Uscita
...
-- 17.07.2015 Daniela: la tabella letta � cambiata, il campo usato � impoatt_no_riacc
  ...
  begin
    ...
    insert into migr_capitolo_entrata
      (capent_id,tipo_capitolo,anno_esercizio,numero_capitolo,numero_articolo,numero_ueb,
	   ...
	   entrata_ricorrente)                                                        -- DAVIDE - 23.08.2016 - aggiunti campi per Capitoli Entrata	  
      (
      -- querey da eliminare a favore della commentata sotto, questa mette in outer join le tabelle di riclassificazione dei capitoli
...
--              cUebAnno3.St_Anno_Prec,cUebAnno3.st_prev, p_ente
              ...
              decode(nvl(cAnno.tipo_entrata,''), 'F' ,'1', 'U', '2', cAnno.tipo_entrata)  -- DAVIDE - 23.08.2016 - aggiunti campi per Capitoli Entrata
    ...
    for migrImpegno in (select i.anno_esercizio,
                        ...
                        --, i.centro_resp centro_resp
                         ...
                         where i.anno_esercizio = p_anno_esercizio
                           ...
                           and I.SDF <> 'S' -- Gli impegni SDF (senza disponibilit� di fondi) non vengoni migrati
                         order by 1, 2, 3) loop
  ...
  begin
    ...
    for migrImpegno in (select i.anno_esercizio,
     ...
     -- DAVIDE - 23.02.016 - segnalazione impegni pluriennali e gestione corretta leggi_provvedimento
      ...
      -- definizione tipo_impegno
      -- verificare se serve , facendo una distinct sul tipo_impegno l'unico valore risultante � S
           if (upper(migrImpegno.tipo_impegno) = 'C' 
                or upper(migrImpegno.tipo_impegno) = 'S'
                or upper(migrImpegno.tipo_impegno) = 'D') then 
                h_tipo_impegno:='SVI';
...
-- DAVIDE - 23.02.016 - segnalazione accertamenti pluriennali e gestione corretta leggi_provvedimento
    ...
    when others then
      ...
      begin
        ...
        if (pEnte is null or pAnnoEsercizio is null) then
            pCodRes := -1;
            pMsgRes := 'proc migrazione_mutuo.Uno o pi� parametri in input non sono stati valorizzati correttamente';
            return;
        end if;
        ...
        exception when others then
               ...
               for migrMutuo in (select 
                            ...
                            if codRes = 0 then
                              ...
                              if codRes = 0 and h_stato_provvedimento is null then
                                -- ci sono 3 casi in produzione per cui lo stato non � definito, quindi � corretto nel caso non lo sia,
                                -- usare lo stato del mutuo?
                                -- il valore NULL � possibile solo su tabelle DELIBERE e non su MOVIMENTI INTERNI
                                h_stato_provvedimento := migrMutuo.stato; -- � cos�?
                              end if;
                            end if;
  ...
  procedure migrazione_voce_mutuo (pAnnoEsercizio varchar2,pEnte number, pCodRes out number, pMsgRes out varchar2,pMutuiInseriti out number,pMutuiScartati out number)
      is
        ...
        h_mutuo varchar2(50) := null;
        h_tipo varchar2(10) := '01'; -- di default il tipo voce mutuo � ORIGINALE, 
      ...
      begin
        ...
        if (pEnte is null or pAnnoEsercizio is null) then
            pCodRes := -1;
            pMsgRes := 'proc migrazione_voce_mutuo.Uno o pi� parametri in input non sono stati valorizzati correttamente';
            return;
        end if;
...
*/
  ...
  IS
		...
		-- Davide - 15.06.2016 - aggiunta numero_provvedimento_calcolato 
		--                       impostato con il numero_provvedimento
        ...
        da_segnalare            number := 0;  -- true: il record � inserito in tab. scarto oltre che in  tab. migrazione 
                                              -- false: il record � inserito in tab.migrazione
        h_sogg_migrato          number := 0;
      ...
      begin
        ...
        if (pEnte is null or pAnnoEsercizio is null) then
            pCodRes := -1;
            pMsgRes := 'proc migrazione_liquidazione.Uno o pi� parametri in input non sono stati valorizzati correttamente';
            return;
        end if;
...
--                               , to_char(l.data_ins,'dd/MM/yyyy') data_emissione
    ...
    -- DAVIDE - 16.12.015 - Fine
                 ...
                 loop
                            ...
                            -- il soggetto non � stato migrato per lo stato?
                            if h_sogg_migrato = 0 then
                              begin
                            ...
                            if codRes = 0 then
                              ...
                              h_nprov_calcolato:=h_numero_provvedimento;      -- Davide - 15.06.2016 - aggiunta numero_provvedimento_calcolato
                              ...
                              ,cofog
                              ,ente_proprietario_id
							  ,numero_provvedimento_calcolato       -- Davide - 15.06.2016 - aggiunta numero_provvedimento_calcolato
    -- DAVIDE - 16.12.015 - Popolamento campo siope_spesa       
                              ,siope_spesa
    -- DAVIDE - 16.12.015 - Fine
                              )
                              values
                                (migr_liquidazione_id_seq.nextval,
                                 ...
                                 migrCursor.cofog,
                                 pEnte
								 ,h_nprov_calcolato           -- Davide - 15.06.2016 - aggiunta numero_provvedimento_calcolato
    -- DAVIDE - 16.12.015 - Popolamento campo siope_spesa       
                            ...
                            if (codRes != 0 or da_segnalare != 0) then
                              ...
                              values
                                (migr_liquid_scarto_id_seq.nextval,
                                 ...
                                 migrCursor.anno_esercizio,
                                 msgMotivoScarto,
                                 decode(da_segnalare,1,'S','N'), -- il record con fl_migrato = S � stato inserito anche nella migr_liquidazione.
                                 pEnte);
  ...
  procedure migrazione_doc_spesa(pEnte number,
      ...
      begin
        ...
        for migrCursor in 
        (select            d.annodoc, d.ndoc,d.tipodoc,
		                   decode(d.tipodoc,'FAT','F',
                                             'NCD','A',
                                             'NTE','A',
                                             d.tipodoc) tipo_fonte,            -- DAVIDE - 16.06.2016 - gestione campo tipo_fonte
		                   d.codben,nvl(d.codben_pagamento,0) codben_pagamento,
          ...
          loop
                          ...
                          if codRes=0 then
                           ...
                           if codRes = 0 then
                              ...
                              (docspesa_id,
                               tipo,
							   tipo_fonte,                      -- DAVIDE - 16.06.2016 - gestione campo tipo_fonte
                               anno,
                               numero,
                               codice_soggetto,
                               codice_soggetto_pag,
                ...
                -- DAVIDE - 05.02.016 - Gestire nuovo campo anno_repertorio               
                              ...
                              (migr_doc_spesa_id_seq.nextval,
                               migrCursor.tipodoc,
                               migrCursor.tipo_fonte,         -- DAVIDE - 16.06.2016 - gestione campo tipo_fonte
                               migrCursor.annodoc,
                               migrCursor.ndoc,
                               migrCursor.codben,
    ...
    begin    
     ...
     exception 
       when others then
        ...
        for migrCursor in 
        (select m.docspesa_id,q.annodoc, q.ndoc, q.tipodoc,
				decode(q.tipodoc,'FAT','F',
                                 'NCD','A',
                                 'NTE','A',
                                 q.tipodoc) tipo_fonte,            -- DAVIDE - 16.06.2016 - gestione campo tipo_fonte
		        q.codben, nvl(d.codben_pagamento,0) codben_pagamento,
        ...
        -- DAVIDE - 10.02.016 - Attributi con date : formato da usare dd/mm/yyyy hh:mm:ss
                ...
                if codRes=0 then
                    ...
                    -- se la quota � incompleta ( non vi sono le imputazioni contabili ) non migriamo il collegamento con la MDP
                    ...
                    if h_modpag_del=0  and h_modpag!=0 then -- DELEGATO NO !
                        begin    
                            ...
                            --select  m.sede_secondaria, mo.soggetto_id into h_sede_secondaria, h_soggetto_id                       -- DAVIDE - 14.07.2016
                            select  m.sede_secondaria, mo.soggetto_id, m.sede_id into h_sede_secondaria, h_soggetto_id, h_sede_id
                    ...
                    if codRes=0 and h_modpag_del!=0 and h_modpag!=0 then -- DELEGATO SI MA NO CESSIONE
                        begin    
                           ...
                           -- into h_sede_secondaria, h_fl_genera_codice,h_soggetto_id
                             select  m.sede_secondaria, mo.soggetto_id, m.sede_id    -- DAVIDE - 14.07.2016
                             into h_sede_secondaria, h_soggetto_id, h_sede_id
                              from migr_modpag m, migr_soggetto mo
                             where mo.codice_soggetto=h_codice_soggetto
                             --and   mo.fl_genera_codice='N' 
                               and   mo.ente_proprietario_id=pEnte
  ...
  */
  procedure migrazione_doc_entrata(pEnte number,
      ...
      begin
        ...
        for migrCursor in 
        (select d.annodoc, d.ndoc,d.tipodoc,
				decode(d.tipodoc,'FTV','F',
                                 'NCV','A',
                                 d.tipodoc) tipo_fonte,            -- DAVIDE - 16.06.2016 - gestione campo tipo_fonte
		        d.codben,nvl(d.codben_incasso,0) codben_incasso,
                decode(d.statodoc,'IN','I',
                                  'LI','L',
                                  'PE','PE',
                                  'PL','PL',
    ...
    begin    
     ...
     exception 
       when others then
        ...
        for migrCursor in 
        (select m.docentrata_id,q.annodoc, q.ndoc, q.tipodoc,
				decode(q.tipodoc,'FTV','F',
                                 'NCV','A',
                                 q.tipodoc) tipo_fonte,            -- DAVIDE - 16.06.2016 - gestione campo tipo_fonte
                q.codben, nvl(d.codben_incasso,0) codben_incasso,
                q.frazione,  
        -- DAVIDE - Conversione Lira / Euro
                d.cod_valuta,
        -- DAVIDE - Fine
