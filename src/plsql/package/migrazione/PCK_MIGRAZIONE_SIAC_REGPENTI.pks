CREATE OR REPLACE PACKAGE PCK_MIGRAZIONE_SIAC AS
    ...
    PROVV_AD CONSTANT VARCHAR2(3):='AD';  -- i provvedimenti di questo tipo hano in chiave la direzione (quindi ricerca su postgres anche per direzione oltre che per numero e anno)
    PROVV_DG  CONSTANT varchar2(2):='DG';
    PROVV_ATTO_LIQUIDAZIONE  CONSTANT varchar2(2):='AL'; -- i provvedimenti di questo tipo hano in chiave la direzione (quindi ricerca su postgres anche per direzione oltre che per numero e anno)
    PROVV_ATTO_LIQUIDAZIONE_SIAC  CONSTANT varchar2(3):='ALG';

    CLASSE_MIGRAZIONE CONSTANT VARCHAR2(250):='MIGRAZIONE||MIGRAZIONE';

-- DAVIDE : MODIFICA TEMPORANEA - CONTROLLO IMPEGNI / ACCERTAMENTI
--          E SCARTO DEI RECORD CON NUMERO PROVVEDIMENTO NON NUMERICO    
    FUNCTION IsNumeric (VALUE IN VARCHAR2) RETURN NUMBER;
-- DAVIDE : fine
