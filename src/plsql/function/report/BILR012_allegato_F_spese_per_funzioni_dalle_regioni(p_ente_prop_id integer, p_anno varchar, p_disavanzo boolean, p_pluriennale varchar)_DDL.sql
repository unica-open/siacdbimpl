...
*/
CREATE OR REPLACE FUNCTION siac."BILR012_allegato_F_spese_per_funzioni_dalle_regioni" (
...
FROM siac_d_class_fam missione_fam, siac_t_class_fam_tree missione_tree,
    ...
    /* 04/08/2016: start filtro per mis-prog-macro*/
...
from siac_t_bil bilancio,
     ...
     ,siac_r_bil_elem_attr attributo_capitolo
...
where 
   ...
   /* capitolo.ente_proprietario_id= bilancio.ente_proprietario_id			and
    ...
    and capitolo.elem_id = attributo_capitolo.elem_id
    and attributo_capitolo."boolean" = 'S'
    and elenco_attributi.attr_id = attributo_capitolo.attr_id
    and	elenco_attributi.attr_code ='FlagFunzioniDelegate'
    ...
    and attributo_capitolo.data_cancellazione       is null
    and elenco_attributi.data_cancellazione 		is null;	

-- carico nella tabella di appoggio con i capitoli gli eventuali 
...
select programma.classif_id, macroaggr.classif_id, p_anno, NULL, prec.elem_code, prec.elem_code2, 
      ...
      and prec.fl_funzionidelegate='S'	
...
from		siac_d_bil_elem_det_tipo capitolo_imp_tipo,
            ...
            FULL join siac_r_bil_elem_attr attributo_capitolo
            on (capitolo_importi.elem_id = attributo_capitolo.elem_id
            	and attributo_capitolo."boolean" = 'S'
            	and	capitolo_importi.ente_proprietario_id = attributo_capitolo.ente_proprietario_id)
            RIGHT join siac_t_attr elenco_attributi
            on (elenco_attributi.attr_id = attributo_capitolo.attr_id
            	and	elenco_attributi.attr_code ='FlagFunzioniDelegate'
                and	elenco_attributi.ente_proprietario_id = attributo_capitolo.ente_proprietario_id )
    where 	capitolo_importi.ente_proprietario_id 	=p_ente_prop_id  
        ...
        --04/08/2016: aggiunto FPV che era gestito con una seconda query 
...
from   
	...
	where			tb1.elem_id	=	tb2.elem_id								and	
        			...
        			tb1.periodo_anno = annoCapImp	AND	tb1.tipo_imp =	TipoImpComp				and	tb1.tipo_capitolo 		in ('STD','FSC') AND
        			tb2.periodo_anno = annoCapImp1	AND	tb1.tipo_imp =	tb2.tipo_imp			and	tb2.tipo_capitolo 		in ('STD','FSC') AND
        			tb3.periodo_anno = annoCapImp2	AND	tb2.tipo_imp =	tb3.tipo_imp			and	tb3.tipo_capitolo 		in ('STD','FSC') AND
        			tb4.periodo_anno = tb1.periodo_anno	AND	tb4.tipo_imp = 	TipoImpRes			and	tb4.tipo_capitolo 		in ('STD','FSC') AND
        			tb5.periodo_anno = tb1.periodo_anno	AND	tb5.tipo_imp = 	TipoImpstanzresidui	and	tb5.tipo_capitolo 		in ('STD','FSC') AND
...
from   
 ...
 /*
      ...
      and	now() at time zone 'CET' between bilancio.validita_inizio and coalesce (bilancio.validita_fine, now() at time zone 'CET')
      and	now() at time zone 'CET' between anno_eserc.validita_inizio and coalesce (anno_eserc.validita_fine, now() at time zone 'CET')
      and	now() at time zone 'CET' between capitolo.validita_inizio and coalesce (capitolo.validita_fine, now() at time zone 'CET')
      and	now() at time zone 'CET' between r_mov_capitolo.validita_inizio and coalesce (r_mov_capitolo.validita_fine, now() at time zone 'CET')
      and	now() at time zone 'CET' between t_capitolo.validita_inizio and coalesce (t_capitolo.validita_fine, now() at time zone 'CET')
      and	now() at time zone 'CET' between movimento.validita_inizio and coalesce (movimento.validita_fine, now() at time zone 'CET')
      and	now() at time zone 'CET' between tipo_mov.validita_inizio and coalesce (tipo_mov.validita_fine, now() at time zone 'CET')
      and	now() at time zone 'CET' between ts_movimento.validita_inizio and coalesce (ts_movimento.validita_fine, now() at time zone 'CET')
      and	now() at time zone 'CET' between r_movimento_stato.validita_inizio and coalesce (r_movimento_stato.validita_fine, now() at time zone 'CET')
      and	now() at time zone 'CET' between tipo_stato.validita_inizio and coalesce (tipo_stato.validita_fine, now() at time zone 'CET')
      and	now() at time zone 'CET' between dt_movimento.validita_inizio and coalesce (dt_movimento.validita_fine, now() at time zone 'CET')
      and	now() at time zone 'CET' between dt_mov_tipo.validita_inizio and coalesce (dt_mov_tipo.validita_fine, now() at time zone 'CET')  
      and	now() at time zone 'CET' between ts_mov_tipo.validita_inizio and coalesce (ts_mov_tipo.validita_fine, now() at time zone 'CET') 
...
if fase_bilancio = 'P' and esiste_siac_t_dicuiimpegnato_bilprev<>1 then
 	for classifBilRec in
...
from siac_rep_mptm_up_cap_importi t1
        ...
        order by missione_code,programma_code,titusc_code,macroag_code
          ...
          stanziamento_fpv_anno2:=classifBilRec.stanziamento_fpv_anno2;
                ...
                -- cerco i dati relativi alle previsioni anno precedente.
