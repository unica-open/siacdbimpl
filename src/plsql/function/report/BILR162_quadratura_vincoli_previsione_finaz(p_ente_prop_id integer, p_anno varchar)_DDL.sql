...
*/
CREATE OR REPLACE FUNCTION siac."BILR162_quadratura_vincoli_previsione_finaz" (
  p_ente_prop_id integer,
  p_anno varchar
)
RETURNS TABLE (
  vincolo_cod varchar,
  tipo_fin_cod varchar,
...
tipo_finanziamento_cap as
(
select rc.elem_id,
       c.classif_id,
       c.classif_code tipo_fin_code,
       c.classif_desc tipo_fin_desc
...
select (case when vincolo_cap.vincolo_code is not null then 'VINCOLATO' else 'NON-VINCOLATO' end) vincolo_code,
       (case when tipo_finanziamento_cap.tipo_fin_code is not null then tipo_finanziamento_cap.tipo_fin_code else 'ND' end ) tipo_fin_code,
...
from sta_anno1,sta_anno2,sta_anno3, sta_cassa_anno1,
     cap
      ...
      left join tipo_finanziamento_cap on ( cap.elem_id=tipo_finanziamento_cap.elem_id )
...
group by (case when vincolo_cap.vincolo_code is not null then 'VINCOLATO' else 'NON-VINCOLATO' end),
         (case when tipo_finanziamento_cap.tipo_fin_code is not null then tipo_finanziamento_cap.tipo_fin_code else 'ND' end )
) CAP_UP_QUERY
-- cap-up 24
),
vincoli_ep as
...
select QUERY_VINCOLI.vincolo_code::varchar vincolo_cod,
       QUERY_VINCOLI.tipo_fin_code::varchar tipo_fin_cod,
       ...
       coalesce(vincoli_ep.stanziamento_cassa_anno1,0) capep_stanziamento_cassa_anno1
from  QUERY_VINCOLI
      left join vincoli_up on (QUERY_VINCOLI.vincolo_code=vincoli_up.vincolo_code and QUERY_VINCOLI.tipo_fin_code=vincoli_up.tipo_fin_code)
      left join vincoli_ep on (QUERY_VINCOLI.vincolo_code=vincoli_ep.vincolo_code and QUERY_VINCOLI.tipo_fin_code=vincoli_ep.tipo_fin_code)
) QUERY_FINALE
order by 1,2;
