...
*/
´╗┐-- Function: "BILR164_calcolo_disavanzo"(integer, character varying)

-- DROP FUNCTION "BILR164_calcolo_disavanzo"(integer, character varying);

CREATE OR REPLACE FUNCTION "BILR164_calcolo_disavanzo"(p_ente_prop_id integer, p_anno character varying)
  RETURNS numeric AS
$BODY$
DECLARE
   importo 	numeric :=0;
...
from 		siac_t_bil_elem_det 		capitolo_importi,
    ...
    where 	capitolo_importi.ente_proprietario_id 	=	p_ente_prop_id  
      ...
      if (importo_imp+importo_fpv) > (importo_acc+importo_ent) then
          importo:=importo_imp+importo_fpv-importo_acc - importo_ent;
	  else 
          importo:=0; 
      end if;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION "BILR164_calcolo_disavanzo"(integer, character varying)
  OWNER TO siac;
GRANT EXECUTE ON FUNCTION "BILR164_calcolo_disavanzo"(integer, character varying) TO public;
GRANT EXECUTE ON FUNCTION "BILR164_calcolo_disavanzo"(integer, character varying) TO siac;
GRANT EXECUTE ON FUNCTION "BILR164_calcolo_disavanzo"(integer, character varying) TO siac_rw;
