...
*/
CREATE OR REPLACE FUNCTION siac."BILR069_Rendiconto_legge421_spese" (
...
RETURNS TABLE (
  ...
  teso_pagamenti_residui numeric,
  teso_pagamenti_competenza numeric,
  cons_pagamenti_residui numeric,
  cons_pagamenti_competenza numeric,
  cons_imp_pagamenti_residui numeric,
  cons_imp_pagamenti_competenza numeric,
  numero_capitoli integer
) AS
$body$
DECLARE
...
from missione , programma,titusc, macroag
    ...
    /* 26/09/2016: nei report di utilità non deve essere inserito 
    	questo filtro */    
...
raise notice 'inserimento tabella di comodo pagamento  ''.';



-----------------------------------------------------------------------------------------------------------------------------------
-------		ACQUISIZIONE DEI PAGAMENTI RESIDUI E IN CONTO COMPETENZA 
-------		PER TESORIERE E CONSIGLIO
-----------------------------------------------------------------------------------------------------------------------------------


insert into siac_rep_pagam_ug_completa
...
from 		siac_t_bil 						bilancio,
    ...
    where 	anno_eserc.anno						= 	p_anno											
        ...
        ------------------------------------------------------------------------------------------		
        --- Estaggo gli STATI 'Q' (Quietanziato = pagamenti del Tesoriere) e		------
        --- 				  'T' (Trasmesso = pagamenti del Consiglio 			    ------		
        --and	stato_ordinativo.ord_stato_code		=	'I' --- 
        	/* Q = pagamenti del tesoriere, T = pagamenti del Consiglio */
        ...
        and	ts_movimento.movgest_id				=	movimento.movgest_id
        	/* anno = quello in input = pagamenti di competenza
               anno < quello in input = pagamenti residui */
        ...
        group by r_capitolo_ordinativo.elem_id,r_capitolo_ordinativo.ente_proprietario_id,
        			movimento.movgest_anno, stato_ordinativo.ord_stato_code;
...
select 	v1.missione_tipo_desc			missione_tipo_desc,
    	...
    	COALESCE(tb2.pagamenti_competenza,0)		pagamenti,
...
from   
	siac_rep_mis_pro_tit_mac_riga_anni v1
           ...
           on    	(v1.programma_id = tb.programma_id    
            ...
            left 	join	siac_rep_pagam_ug_completa			tb2		on tb2.elem_id	=	tb.elem_id
...
pagamenti_residui:=classifBilRec.residui_passivi;
pagamenti_competenza:=classifBilRec.previsioni_definitive_comp;


	/* Avanzamento QUIETANZIATO = conto del Tesoriere */
IF classifBilRec.STATO_AVANZAM_PAG = 'Q' THEN
		/* anno = a quello richiesto: COMPETENZA */        
	IF classifBilRec.STATO_AVANZAM_PAG = annoCapImp_int THEN
		teso_pagamenti_residui:=0;
		teso_pagamenti_competenza:=classifBilRec.pagamenti;
		cons_pagamenti_residui:=0;
		cons_pagamenti_competenza:=0;
    ELSE
		teso_pagamenti_residui:=classifBilRec.pagamenti;
		teso_pagamenti_competenza:=0;
		cons_pagamenti_residui:=0;
		cons_pagamenti_competenza:=0;
    END IF;    	
	/* Avanzamento TRASMESSO = conto del Consiglio  */    
ELSIF classifBilRec.STATO_AVANZAM_PAG = 'T' THEN	
		/* anno = a quello richiesto: COMPETENZA */        
	IF classifBilRec.STATO_AVANZAM_PAG = annoCapImp_int THEN
		teso_pagamenti_residui:=0;
		teso_pagamenti_competenza:=0;
		cons_pagamenti_residui:=0;
		cons_pagamenti_competenza:=classifBilRec.pagamenti;
    ELSE
		teso_pagamenti_residui:=0;
		teso_pagamenti_competenza:=0;
		cons_pagamenti_residui:=classifBilRec.pagamenti;
		cons_pagamenti_competenza:=0;
    END IF;
ELSE
		teso_pagamenti_residui:=0;
...
END IF;
	...
	/* Se l'anno di competenza è uguale a quello richiesto, gli impegni sono quelli 
    	di competenza */
if classifBilRec.ANNO_IMPEGNO::integer = annoCapImp_int THEN
	cons_imp_pagamenti_residui:=0;
	cons_imp_pagamenti_competenza:=classifBilRec.impegni;
	/* Se l'anno di competenza è minore di quello richiesto, gli impegni sono quelli 
    	residui */    
ELSIF classifBilRec.ANNO_IMPEGNO::integer < annoCapImp_int THEN
	cons_imp_pagamenti_residui:=classifBilRec.impegni;
	cons_imp_pagamenti_competenza:=0;
ELSE
	cons_imp_pagamenti_residui:=0;
	cons_imp_pagamenti_competenza:=0;
END IF;
	...
	BEGIN
        ...
        INTO numCapitoli
           ...
           on    	(v1.programma_id = tb.programma_id    
            ...
            left 	join	siac_rep_pagam_ug_completa			tb2		on tb2.elem_id	=	tb.elem_id
            left    join 	siac_rep_impegni			tb3		on tb3.elem_id	=	tb.elem_id
		where     v1.titusc_code =classifBilRec.titusc_code
			AND  v1.missione_code =classifBilRec.missione_code
...
*/
	...
	/* conto il numero di articoli per capitolo */
IF classifBilRec.bil_ele_code IS NOT NULL THEN
	BEGIN
    	select 	count(distinct tb.elem_code2 )
        INTO numCapitoli
...
raise notice 'ora: % ',clock_timestamp()::varchar;
 ...
 RTN_MESSAGGIO:='FINE estrazione dei dati dalle tabelle di comodo ''.'; 
raise notice 'FINE estrazione dei dati dalle tabelle di comodo ''.';
...
delete from siac_rep_pagam_ug_completa 				where utente=user_table;
