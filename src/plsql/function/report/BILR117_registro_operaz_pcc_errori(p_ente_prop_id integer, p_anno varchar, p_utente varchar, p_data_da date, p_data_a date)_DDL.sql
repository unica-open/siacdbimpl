...
*/
CREATE OR REPLACE FUNCTION siac."BILR117_registro_operaz_pcc_errori" (
...
select t_ente.codice_fiscale cod_fisc_ente_dest, t_ente.ente_denominazione,
    ...
    t_registro_pcc.rpcc_esito_code , t_registro_pcc.rpcc_esito_desc,
     ...
     ,t_registro_pcc.rpcc_id_trans_pa
...
where t_ente.ente_proprietario_id=t_registro_pcc.ente_proprietario_id
	...
	AND t_ente.ente_proprietario_id=p_ente_prop_id   
    	/* devo estrarre tutte le tipologie */ 
    --AND d_pcc_oper_tipo.pccop_tipo_code='CP'
    AND d_pcc_oper_tipo.pccop_tipo_code in ('CO', 'CS', 'CP')
    AND (t_registro_pcc.rpcc_esito_code IS NOT NULL AND
    		t_registro_pcc.rpcc_esito_code <> '000')
    	/* 23/11/2016: fatta la date_trunc perchè se le date DA e A sono uguali
        	non erano estratti record.
            Usata la data rpcc_registrazione_data invece che la data_creazione        
        */
    AND (p_data_da IS NOT NULL AND p_data_a IS NOT NULL 
            	--AND (t_registro_pcc.data_creazione between  p_data_da AND
                --	p_data_a ))    
                AND (date_trunc ('day',t_registro_pcc.rpcc_registrazione_data) BETWEEN
                     p_data_da and p_data_a))
...
loop
    ...
    numero_quota=elencoRegistriRec.subdoc_numero;
    	--16/12/2016: dall'importo della quota deve essere sottratto l'importo
        -- split_reverse della quaota stessa.
...
--    estremi_impegno=to_date(elencoRegistriRec.data_emissione_imp ::VARCHAR,'dd/MM/yyyy') ||'-'||elencoRegistriRec.anno_impegno ::VARCHAR ||'-'||elencoRegistriRec.numero_impegno ::VARCHAR;
    ...
    id_trans_pa=elencoRegistriRec.rpcc_id_trans_pa;
