...
*/
CREATE OR REPLACE FUNCTION siac."BILR010_Allegato_E_Utilizzo_contributi_trasferimenti_UE_e_inter" (
...
select programma.classif_id, macroaggr.classif_id, p_anno, NULL, prec.elem_code, prec.elem_code2, 
    ...
    --03/08/2016: aggiunto FPVC      
      ...
      and prec.fl_trasferimentiue<>'8'	
      and prec.fl_trasferimentiue is not null	
...
and a.fl_trasferimentiue <> '8'	
and a.fl_trasferimentiue is not null
and a.fl_trasferimentiue <> ''
)
, progr as (
...
from   
	...
	where			tb1.elem_id	=	tb2.elem_id								and	
                    ...
                    AND
        			tb2.periodo_anno = annoCapImp1	AND	tb1.tipo_imp =	tb2.tipo_imp		and	tb1.tipo_capitolo 		in ('STD','FSC')	
                    AND
        			tb3.periodo_anno = annoCapImp2	AND	tb2.tipo_imp =	tb3.tipo_imp		and	tb1.tipo_capitolo 		in ('STD','FSC')
                    AND
        			tb4.periodo_anno = tb1.periodo_anno	AND	tb4.tipo_imp = 	TipoImpRes		and	tb1.tipo_capitolo 		in ('STD','FSC')
                    AND
        			tb5.periodo_anno = tb1.periodo_anno	AND	tb5.tipo_imp = 	TipoImpstanzresidui		and	tb1.tipo_capitolo 		in ('STD','FSC')	
                    AND
        			tb6.periodo_anno = tb1.periodo_anno	AND	tb6.tipo_imp = 	TipoImpCassa		and	tb1.tipo_capitolo 		in ('STD','FSC')
...
from   
 ...
 /*
      ...
      and	now() between r_mov_capitolo.validita_inizio and coalesce (r_mov_capitolo.validita_fine, now() at time zone 'CET')
      and	now() between t_capitolo.validita_inizio and coalesce (t_capitolo.validita_fine, now() at time zone 'CET')
      and	now() between movimento.validita_inizio and coalesce (movimento.validita_fine, now() at time zone 'CET')
      and	now() between tipo_mov.validita_inizio and coalesce (tipo_mov.validita_fine, now() at time zone 'CET')
      and	now() between ts_movimento.validita_inizio and coalesce (ts_movimento.validita_fine, now() at time zone 'CET')
      and	now() between r_movimento_stato.validita_inizio and coalesce (r_movimento_stato.validita_fine, now() at time zone 'CET')
      and	now() between tipo_stato.validita_inizio and coalesce (tipo_stato.validita_fine, now() at time zone 'CET')
      and	now() between dt_movimento.validita_inizio and coalesce (dt_movimento.validita_fine, now() at time zone 'CET')
      and	now() between dt_mov_tipo.validita_inizio and coalesce (dt_mov_tipo.validita_fine, now() at time zone 'CET')  
      and	now() between ts_mov_tipo.validita_inizio and coalesce (ts_mov_tipo.validita_fine, now() at time zone 'CET') 
...
-- 09/09/2016: questo controllo non serve più
...
from siac_rep_mptm_up_cap_importi t1
        ...
        -- 09/09/2016: deve puntare ad elem_id e non ad elem_id_old
        --on (t1.elem_id_old	=	t2.elem_id
        on (t1.elem_id	=	t2.elem_id
        	and(t1.stanziamento_prev_anno >	0 	or
            	t1.stanziamento_prev_anno1 > 0	or
                t1.stanziamento_prev_anno2 > 0))
     ...
     /*    and	(
      ...
      IF bil_ele_code IS NOT NULL THEN
        ...
        raise notice 'YYYY tipo_categ_capitolo = %', tipo_categ_capitolo;
            ...
            IF tipo_categ_capitolo= 'FPV' THEN
              stanziamento_anno_prec=0;
