...
*/
CREATE OR REPLACE FUNCTION siac."BILR069_Rendiconto_legge421_entrate" (
...
RETURNS TABLE (
  ...
  teso_riscossioni_residui numeric,
  teso_riscossioni_competenza numeric,
  cons_riscossioni_residui numeric,
  cons_riscossioni_competenza numeric,
  cons_accertamenti_residui numeric,
  cons_accertamenti_competenza numeric,
  numero_capitoli numeric,
  nome_ente varchar
) AS
$body$
DECLARE
...
-----------------------------------------------------------------------------------------------------------------------------------
-------		ACUISQIZIONE DELLE RISCOSSIONI IN CONTO RESIDUI (riferimento report RS)
-------		ACQUISIZIONE DELLE RISCOSSIONI IN CONTO COMPETENZA (riferimento report CP)
-----------------------------------------------------------------------------------------------------------------------------------

RTN_MESSAGGIO:='inserimento tabella di comodo riscossioni residui e competenza ''.';
raise notice 'inserimento tabella di comodo riscossioni residui e competenza  ';



/* su questa tabella carico i dati delle "RISCOSSIONI" per 
	- "CONTO DEL TESORIERE" (stato ordinativi = QUIETANZIATO) 
	- "DETERMINAZIONI DEL CONSIGLIO" (stato ordinativi = TRASMESSO) */
insert into siac_rep_riscos_eg_completa
select 		r_capitolo_ordinativo.elem_id,
            sum(ordinativo_imp.ord_ts_det_importo),
            stato_ordinativo.ord_stato_code,
...
from 		siac_t_bil 						bilancio,
    ...
    where 	anno_eserc.anno						= 	p_anno											
        ...
        and	stato_ordinativo.ord_stato_code		in('Q', 'T') --- 
        -----------------------------------------------------------------------------------------------
        ...
        group by r_capitolo_ordinativo.elem_id,r_capitolo_ordinativo.ente_proprietario_id,
        		stato_ordinativo.ord_stato_code, movimento.movgest_anno;
...
insert into siac_rep_accertamenti_completa
select 	tb2.elem_id,
		tb.importo,
        tb.stato_code,
        tb.anno_competenza,
...
tipo_stato.movgest_stato_code stato_code,
movimento.movgest_anno anno_competenza
    from 
      siac_t_bil      bilancio, 
      siac_t_periodo     anno_eserc, 
      siac_t_bil_elem     capitolo , 
      ...
      /* devo usare questa variabile intera senza fare il cast perchè altrimenti la query 
         si blocca se uso movimento.movgest_anno nella GROUP BY */ 
      and movimento.movgest_anno   	<= 	annoCapImp_int
      and movimento.bil_id					=	bilancio.bil_id
...
group by capitolo.elem_id, tipo_stato.movgest_stato_code, movimento.movgest_anno)
tb 
,
...
select 	v1.classif_tipo_desc1    		titoloe_TIPO_DESC,
        ...
        tb2.stato						STATO_AVANZAM_RISCOS,
        tb2.anno_competenza				ANNO_COMPETENZA_RISCOS,
        tb3.stato						STATO_AVANZAM_ACCERT,
        tb3.anno_competenza				ANNO_COMPETENZA_ACCERT,
        ...
        COALESCE(tb2.riscossioni,0)					riscossioni,--riscossioni_competenza,
...
from  	siac_rep_tit_tip_cat_riga_anni v1
           ...
           LEFT join	siac_rep_riscos_eg_completa 			tb2		on 	tb2.elem_id	=	tb.elem_id
           left join    siac_rep_accertamenti_completa		tb3		on  tb3.elem_id	=	tb.elem_id         	
...
/* accertamenti: in base all'anno di competenza discrimino se si tratta di residui o competenza:
	- ANNO < dell'anno richiesto = RESIDUI;
    - ANNO = dell'anno richiesto = COMPETENZA
*/    
IF classifBilRec.ANNO_COMPETENZA_ACCERT = annoCapImp_int THEN
	cons_accertamenti_residui:=0;
	cons_accertamenti_competenza:=classifBilRec.accertamenti;
ELSIF classifBilRec.ANNO_COMPETENZA_ACCERT < annoCapImp_int THEN
	cons_accertamenti_residui:=classifBilRec.accertamenti;
	cons_accertamenti_competenza:=0;
ELSE
	cons_accertamenti_residui:=0;
	cons_accertamenti_competenza:=0;    
END IF;

	/* Avanzamento QUIETANZIATO = conto del Tesoriere */
IF classifBilRec.STATO_AVANZAM_RISCOS = 'Q' THEN
		/* anno = a quello richiesto: COMPETENZA */        
	IF classifBilRec.ANNO_COMPETENZA_RISCOS = annoCapImp_int THEN
    	teso_riscossioni_residui:=0;
		teso_riscossioni_competenza:=classifBilRec.riscossioni;
        cons_riscossioni_residui:=0;
        cons_riscossioni_competenza:=0;
    ELSE
    	teso_riscossioni_residui:=classifBilRec.riscossioni;
		teso_riscossioni_competenza:=0;
        cons_riscossioni_residui:=0;
        cons_riscossioni_competenza:=0;
    END IF;    	
	/* Avanzamento TRASMESSO = conto del Consiglio  */    
ELSIF classifBilRec.STATO_AVANZAM_RISCOS = 'T' THEN	
		/* anno = a quello richiesto: COMPETENZA */        
	IF classifBilRec.ANNO_COMPETENZA_RISCOS = annoCapImp_int THEN
    	teso_riscossioni_residui:=0;
		teso_riscossioni_competenza:=0;
        cons_riscossioni_residui:=0;
        cons_riscossioni_competenza:=classifBilRec.riscossioni;
    ELSE
    	teso_riscossioni_residui:=0;
		teso_riscossioni_competenza:=0;
        cons_riscossioni_residui:=classifBilRec.riscossioni;
        cons_riscossioni_competenza:=0; 
    END IF;
ELSE
    teso_riscossioni_residui:=0;
...
IF classifBilRec.bil_ele_code IS NOT NULL THEN
	BEGIN
    	select 	count(distinct tb.elem_code2 )
        INTO numCapitoli
			from  	siac_rep_tit_tip_cat_riga_anni v1
			left  join siac_rep_cap_eg tb
           on    	(v1.categoria_id = tb.classif_id    
           ...
           LEFT join	siac_rep_riscos_eg_completa 			tb2		on 	tb2.elem_id	=	tb.elem_id
           left join    siac_rep_accertamenti_completa		tb3		on  tb3.elem_id	=	tb.elem_id     
		where     v1.titolo_code =classifBilRec.titoloe_CODE
			AND  v1.tipologia_code =classifBilRec.tipologia_code
        	--AND  v1.programma_code=classifBilRec.programma_code     
       	 	AND  tb.elem_code =classifBilRec.bil_ele_code  
            and tb.elem_id IS NOT NULL;
               IF NOT FOUND THEN
                  RAISE EXCEPTION 'Non esiste il capitolo %', classifBilRec.bil_ele_code;
                  return;
                ELSE
                    numero_capitoli= numCapitoli;
                END IF;
          END;
END IF;
...
delete from siac_rep_riscos_eg_completa				where utente=user_table;

delete from siac_rep_accertamenti_completa			where utente=user_table;
