...
*/
CREATE OR REPLACE FUNCTION siac."BILR114_Prosp_entrate_tit_tipol_categ" (
...
------		E DELLE PREVISIONI DEFINITIVE DI CASSA 
---------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------------------------------------------
-------		ACQUISIZIONE DELLE RISCOSSIONI IN CONTO RESIDUI
------------------------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------------------
-------		ACQUISIZIONE DELLE RISCOSSIONI IN CONTO COMPETENZA
-----------------------------------------------------------------------------------------------------------------------------------
...
--where tb.elem_code is not null
...
loop
	...
	IF classifBilRec.BIL_ELE_ID IS NOT NULL THEN
          ricorrente='';
          select distinct t_class.classif_desc
          INTO ricorrente
          from 
          ...
          where  d_class_tipo.classif_tipo_id=t_class.classif_tipo_id
              ...
              and d_class_tipo.classif_tipo_code='RICORRENTE_ENTRATA'
              and r_bil_elem_class.data_cancellazione IS NULL
              and d_class_tipo.data_cancellazione IS NULL
              and t_class.data_cancellazione IS NULL;
          IF NOT FOUND THEN
          	flag_ricorrente = 'N';
          ELSIF upper(ricorrente) = 'RICORRENTE' THEN
          	flag_ricorrente = 'S';
          ELSE
          	flag_ricorrente = 'N';
          END IF;
...
if flag_ricorrente='S' THEN
	...
	accert_non_ricorrenti=accertamenti;
end if;
