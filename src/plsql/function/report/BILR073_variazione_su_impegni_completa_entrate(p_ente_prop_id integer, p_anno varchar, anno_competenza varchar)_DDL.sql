...
*/
CREATE OR REPLACE FUNCTION siac."BILR073_variazione_su_impegni_completa_entrate" (
...
elemTipoCode:='CAP-EG'; -- tipo capitolo entrata gestione      
...
BEGIN
		/* se l'anno è specificato inserisco le condizioni per estrarre solo i dati che
         riguardano quell'anno di bilancio */
	...
	for elencoVariazioni in   
      ...
      t_mov_gest.movgest_numero NUM_ACCERT,t_mov_gest.movgest_anno ANNO_COMP_ACCERT,
...
loop
	...
	anno_accert=elencoVariazioni.ANNO_COMP_ACCERT;
    numero_accert=elencoVariazioni.NUM_ACCERT;
...
else /* anno di competenza NON specificato: estraggo i dati di tutti gli anni */
BEGIN
    ...
    where d_mov_gest_tipo.movgest_tipo_id=t_mov_gest.movgest_tipo_id
            ...
            and ts_mov_tipo.movgest_ts_tipo_code  = 'T' 
            and stato_capitolo.elem_stato_code='VA'
            	/* estraggo tutti gli anni di competenza dei movimenti */
            and t_mov_gest.movgest_anno <=annoCompetenza_int
            AND t_mov_gest.ente_proprietario_id=p_ente_prop_id
            and d_class_tipo.classif_tipo_desc='Cdc(Settore)'
            and t_mov_gest.data_cancellazione is NULL
...
raise notice 'fine estrazione delle variazioni per le entrate';
