...
*/
CREATE OR REPLACE FUNCTION siac."BILR126_stampa_avviso_pagamento_ABI36" (
...
quietanzante_delegato='';

anno_eser_int=p_anno ::INTEGER;

/* 21/10/2016: procedura nata come copia della BILR113_stampa_avviso_pagamento,
	per gestire la stampa dell'avviso di pagamento degli enti ABI36.
    Le modifiche apportare sono:
    - invertito il controllo sul campo flusso_elab_mif_tipo_dec della
      tabella mif_d_flusso_elaborato_tipo che identifica se un ente è
      ABI36 o meno.
    - gli importi sono presi come sono e non sono divisi per 100 perchè
      hanno la cifra decimale.
    - diverso modo di recuperare gli impegni.
*/
...
select 	distinct os.mif_ord_desc_ente, ep.codice_fiscale cod_fisc_ente, os.mif_ord_anno_esercizio,
        ...
        WHERE  os.ente_proprietario_id=ep.ente_proprietario_id
            ...
            AND flusso_elab_tipo.data_cancellazione IS NULL 
        		/* non è necessario testare la data di
...
if elencoMandati.flusso_elab_mif_tipo_dec = false AND contaRecord=1 THEN
	display_error='Stampa avviso di pagamento ABI36 (BILR126): STAMPA NON UTILIZZABILE.';
   -- raise notice 'display_error = %', display_error;
    return next;
    return;
end if;
...
--raise notice 'mif_ord_ord_id = %, ord_ts_id = %, FLUSSO_ID= %, max_data= %, data_mandato = % ', elencoMandati.mif_ord_ord_id, elencoMandati.ord_ts_id, elencoMandati.flusso_elab_mif_id,elencoMandati.flusso_elab_mif_data,elencoMandati.mif_ord_data  ;
	 ...
	 /* ci sono enti per cui lo stato non è "I" ma "INSERIMENTO".
...
IF upper(substr(elencoMandati.mif_ord_codice_funzione,1,1))='I' THEN 
    ...
    --importo_tot_mandati_dopo_emiss= COALESCE(elencoMandati.mif_ord_mandati_prev ::numeric,0);
    ...
    /* cerco impegno e sub-impegno: potrebbero essere più di uno, quindi
...
for elencoImpegni in	
 select d_ord_tipo.ord_tipo_code, t_ord_ts.ord_id, t_ord.ord_numero, t_ord.ord_anno,
   ...
   IF COALESCE(elencoMandati.doc_numero,'') != '' THEN     	
        ...
        /* per problemi di visualizzazione nel report, nel caso 
           il numero fattura sia troppo lungo inserisco degli spazi
           in modo che il report possa portare a capo e non tagliare.
           GLi spazi sono inseriti solo se la stringa non contiene già
           spazi o / perchè in questo caso il report è in grado di mettere
           a capo la stringa.  */
    ...
    if elencoMandati.mif_ord_ord_id IS NOT NULL THEN
        ...
        /* L'IBAN deve essere estratto dalle modalità di pagamento (non dal MIF),
            sia per bonifici italiani che esteri.            
            Cerco le modalità di pagamento e IBAN*/
        /* 24/11/2016: aggiunto il quietanzante che è presente e sarà 
        	visualizzato sul report nel caso la modalità di pagamrento sia
            'Persona delegata alla quietanza' */
        BEGIN
            select d_accredito_tipo.accredito_tipo_desc, COALESCE(t_modpag.iban,''),
            COALESCE(t_modpag.quietanziante,'')
            INTO desc_mod_pagamento, banca_iban, quietanzante_delegato
        ...
        END;    
			...
			/* In caso di cessione di incasso su siac_r_ordinativo_modpag
            	non è valorizzata la modalità di pagamento.
                Devo cercare quella del soggetto a cui è stato ceduto l'incasso. */
