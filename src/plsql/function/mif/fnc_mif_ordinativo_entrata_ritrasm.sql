...
*/
*drop FUNCTION fnc_mif_ordinativo_entrata_ritrasm
...
CREATE OR REPLACE FUNCTION fnc_mif_ordinativo_entrata_ritrasm
( enteProprietarioId integer,
  nomeEnte VARCHAR,
  annobilancio varchar,
  ordArray text,
...
BEGIN
    ...
    if codiceRisultato=0 and coalesce(mifOrdRitrasmRec.mifOrdRitrasm,0)=0 then
     ...
     if codiceRisultato=0 and coalesce(mifOrdRitrasmRec.mifOrdRitrasm,0)!=0 then
     strMessaggio:='Richiamo fnc_mif_ordinativo_entrata interno mifOrdRitrasmElabId='||mifOrdRitrasmRec.mifOrdRitrasmElabId||'.';
