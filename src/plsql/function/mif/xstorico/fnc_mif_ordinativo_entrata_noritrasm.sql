...
*/
- 04.04.2016 Sofia - versione iniziale e attualmente in prod senza parametro di ritrasmission
-- senza join con mif_t_ordinativo_ritrasmesso
-- versione in prod
...
/*    select 1 into codResult
    ...
    ( select ord.ord_id, FUNZIONE_CODE_I, bil.bil_id,per.periodo_id,per.anno::integer,
             ...
             0,0,elem.elem_id,0,0,0,ord.contotes_id,
             ord.notetes_id,ord.dist_id, ord.ord_desc,
...
--     execute  'ANALYZE mif_t_ordinativo_entrata_id;';
	  ...
	  (select ord.ord_id, FUNZIONE_CODE_S, bil.bil_id,per.periodo_id,per.anno::integer,
              ...
              lpad(extract('month' from ord.ord_emissione_data)::varchar,2,'0')||'-'||
              lpad(extract('day' from ord.ord_emissione_data)::varchar,2,'0'),0,
              0,0,elem.elem_id,0,0,0,ord.contotes_id,ord.notetes_id,ord.dist_id, ord.ord_desc,
              ord.ord_cast_cassa,ord.ord_cast_competenza,ord.ord_cast_emessi,
              ord.login_creazione, ord.login_modifica,
              enteProprietarioId,loginOperazione
         ...
         if flussoElabMifElabRec.flussoElabMifElab=true then
			if coalesce(flussoElabMifElabRec.flussoElabMifDef,NVL_STR)!=NVL_STR then
              ...
              if coalesce(flussoElabMifElabRec.flussoElabMifParam,NVL_STR)!=NVL_STR then
                   ...
                   if valFruttiferoId is not null then
                    strMessaggio:='Lettura dati ordinativo numero='||mifOrdinativoIdRec.mif_ord_ord_numero
                       ||' annoBilancio='||mifOrdinativoIdRec.mif_ord_anno_bil
                       ||' ord_id='||mifOrdinativoIdRec.mif_ord_ord_id
                       ||' mif_ord_id='||mifOrdinativoIdRec.mif_ord_id
...
--    execute  'ANALYZE mif_t_ordinativo_entrata_ricevute;';
   ...
   set (flusso_elab_mif_id_flusso_oil,flusso_elab_mif_num_ord_elab,flusso_elab_mif_file_nome,flusso_elab_mif_esito_msg)=
   	   (flussoElabMifOilId,numeroOrdinativiTrasm,flussoElabMifTipoNomeFile||'_'||enteOilRec.ente_oil_codice,
