...
-- 20.01.2017 Sofia - adeguato query di estrazione classificatori e varie per utilizzo di dataFinVal errata
-- 10.02.2017 Sofia - JIRA-SIAC-4423 - gestione tag  destinazione_ente_pagante, Vincolato per CmTo
-- 16.02.2017 Sofia HD-INC000001564624 e HD-INC000001562461
-- 21.02.2017 Sofia HD-INC000001573104
...
-- FLUSSO_MIF_ELAB_FLAG_COPERTURA CONSTANT integer:=122;
 ...
 FLUSSO_MIF_ELAB_COD_RAGGRUP    CONSTANT integer:=100; -- 21.02.2017 Sofia aggiunta gestione codice_raggruppamento per CmTo
...
--	        	    and date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(c.validita_fine,dataFineVal)) 19.01.2017
            	 ...
            	 -- 20.03.2017  	JIRA-SIAC-4621-CMTO
...
--        raise notice 'messaggio=%',strMessaggio;
        ...
        -- 21.02.2017 Sofia CmTo
        -- <codice_raggruppamento>
        flussoElabMifElabRec:=null;
        flussoElabMifValore:=null;
    --    mifCountRec:=mifCountRec+1;
        flussoElabMifElabRec:=mifFlussoElabMifArr[FLUSSO_MIF_ELAB_COD_RAGGRUP];
        strMessaggio:='Lettura dati ordinativo numero='||mifOrdinativoIdRec.mif_ord_ord_numero
                       ||' annoBilancio='||mifOrdinativoIdRec.mif_ord_anno_bil
                       ||' ord_id='||mifOrdinativoIdRec.mif_ord_ord_id
                       ||' mif_ord_id='||mifOrdinativoIdRec.mif_ord_id
                       ||'. Lettura dati configurazione per campo '||flussoElabMifElabRec.flusso_elab_mif_campo
                       ||' mifCountRec='||FLUSSO_MIF_ELAB_COD_RAGGRUP
                       ||' tipo flusso '||MANDMIF_TIPO||'.';
        if flussoElabMifElabRec.flussoElabMifId is null then
            RAISE EXCEPTION ' Configurazione tag/campo non presente in archivio.';
        end if;
        ...
        if flussoElabMifElabRec.flussoElabMifAttivo=true then
         if flussoElabMifElabRec.flussoElabMifElab=true then
         		if mifOrdinativoIdRec.mif_ord_dist_id is not null then
                    ...
                    where d.dist_id=mifOrdinativoIdRec.mif_ord_dist_id;
                    if flussoElabMifValore is not null then
                    	mifFlussoOrdinativoRec.mif_ord_cod_raggrup:=flussoElabMifValore;
                    end if;
                end if;
         else
            RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
        	...
        	if codiceCge is null then -- 4
                ...
                if bAvvioSiopeNew = true then
                  ...
                  if codiceFinVTbr is not null and codiceFinVTipoTbrId is null then --1
						...
						and   tipo.data_cancellazione is null
						and   date_trunc('day',dataElaborazione)>=date_trunc('day',validita_inizio)
--						and   date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(validita_fine,dataFineVal)); 19.01.2017 QUI QUI QUI
						and   date_trunc('day',dataElaborazione)<=date_trunc('day',coalesce(validita_fine,dataElaborazione));
                  end if; --1
                  ...
                  if codiceFinVTipoTbrId is not null then --2
                    ...
                    into flussoElabMifValore,flussoElabMifValoreDesc
		 	        ...
		 	        and   r.data_cancellazione is null
				    and   r.validita_fine is NULL
	  		        and   class.data_cancellazione is null; -- 20.01.2017
--			        and date_trunc('day',dataElaborazione)>=date_trunc('day',class.validita_inizio) 20.01.2017
--	         	    and date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(class.validita_fine,dataFineVal)); 20.01.2017
...
---      	mifFlussoOrdinativoRec.mif_ord_siope_descri_cge:=descCge; 25.02.2016 Sofia
  ...
  strMessaggio:='Inserimento mif_t_ordinativo_entrata per ord. numero='||mifOrdinativoIdRec.mif_ord_ord_numero
        ...
        (
		 ...
		 mif_ord_cod_raggrup,      -- 21.02.2017 Sofia CmTo
		 mif_ord_progr_vers,
...
--  		annoBilancio||flussoElabMifDistOilId::varchar, -- flussoElabMifDistOilId -- 18.04.2016 Sofia
        ...
        mifFlussoOrdinativoRec.mif_ord_cod_raggrup,       -- mif_ord_cod_raggrup -- 21.02.2017 Sofia CmTo
     ...
     if flussoElabMifElabRec.flussoElabMifAttivo=true then
     	if flussoElabMifElabRec.flussoElabMifElab=true then
            ...
            if capOrigAttrId is null then
                ...
                and   attr.data_cancellazione is null
      			and   date_trunc('day',dataElaborazione)>=date_trunc('day',attr.validita_inizio)
-- 	 			and	  date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(attr.validita_fine,dataFineVal)); 20.01.2017
 	 			and	  date_trunc('day',dataElaborazione)<=date_trunc('day',coalesce(attr.validita_fine,dataElaborazione));
            end if;
       ...
       if flussoElabMifElabRec.flussoElabMifAttivo=true then
         ...
         if flussoElabMifElabRec.flussoElabMifParam is not null then
            ...
            if classifCdrTipoId is null and classifCdrTipoCode is not null then
		        ...
		        and   tipo.data_cancellazione is null
			    and   date_trunc('day',dataElaborazione)>=date_trunc('day',tipo.validita_inizio)
-- 				and   date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(tipo.validita_fine,dataFineVal)); 20.01.2017
 				and   date_trunc('day',dataElaborazione)<=date_trunc('day',coalesce(tipo.validita_fine,dataElaborazione));
            end if;
...
--			 	and	  date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(class.validita_fine,dataFineVal)) 20.01.2017
                and   rclass.data_cancellazione is null
                and   rclass.validita_fine is null -- 20.01.2017
--			    and   date_trunc('day',dataElaborazione)>=date_trunc('day',rclass.validita_inizio) 20.01.2017
--			 	and	  date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(rclass.validita_fine,dataFineVal)) 20.01.2017
...
--		and	  date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(conto.validita_fine,dataFineVal)) 20.01.2017
...
--	    and	  date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(evento.validita_fine,dataFineVal)); 20.01.2017
        ...
        if 	flussoElabMifValore is  null then
         ...
         if codiceFinVTbr is not null and codiceFinVTipoTbrId is null then
			...
			and   tipo.data_cancellazione is null
			and   date_trunc('day',dataElaborazione)>=date_trunc('day',validita_inizio)
--			and   date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(validita_fine,dataFineVal)); 20.01.2017
			and   date_trunc('day',dataElaborazione)<=date_trunc('day',coalesce(validita_fine,dataElaborazione));
         end if;
...
--		and	  date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(class.validita_fine,dataFineVal)) 20.01.2017
   ...
   if aslTipoTbrId is not null then
       ...
       and   class.data_cancellazione is null; -- 20.01.2017
	   -- Sofia 10.11.2016 INC000001359464
--	   and date_trunc('day',dataElaborazione)>=date_trunc('day',class.validita_inizio) 20.012.2017
--  	   and date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(class.validita_fine,dataFineVal)); 20.01.2017
--       and   class.validita_fine is null;
