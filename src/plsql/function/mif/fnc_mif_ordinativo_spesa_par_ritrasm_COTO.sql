...
-- ordinativoTsDetTipoId integer:=null;
 ...
 -- 27.09.2016 Sofia COTO
 transElemCoTo varchar(100):=null;
 ...
 isTransElemCoToAttiva boolean :=false; -- 27.09.2016 Sofia COTO
 ...
 FLUSSO_MIF_ELAB_DISP_BENEF_COTO CONSTANT integer:=199; -- 05.09.2016 Sofia COTO
 FLUSSO_MIF_ELAB_DISP_BENEF_COTO_TRB CONSTANT integer:=201; -- 05.09.2016 Sofia COTO
...
--    execute  'ANALYZE mif_t_ordinativo_spesa_id;';
   ...
   flussoElabMifElabRec:=null;
   flussoElabMifElabRec:=mifFlussoElabMifArr[FLUSSO_MIF_ELAB_DISP_BENEF_COTO_TRB];
   strMessaggio:='Lettura dati configurazione per campo '||flussoElabMifElabRec.flusso_elab_mif_campo
   ...
   flussoElabMifElabRec:=mifFlussoElabMifArr[FLUSSO_MIF_ELAB_TBR];
    ...
    if flussoElabMifElabRec.flussoElabMifAttivo=true or isTransElemCoToAttiva=true then
    	if flussoElabMifElabRec.flussoElabMifElab=true or isTransElemCoToAttiva=true then
...
/*        strMessaggio:='Lettura dati ordinativo numero='||mifOrdinativoIdRec.mif_ord_ord_numero
        ...
        if flussoElabMifElabRec.flussoElabMifAttivo=true or isTransElemAttiva=true or isTransElemCoToAttiva=true then
         if flussoElabMifElabRec.flussoElabMifElab=true or isTransElemAttiva=true or isTransElemCoToAttiva=true then
         		if flussoElabMifElabRec.flussoElabMifParam is not null or isTransElemAttiva=true then
                    ...
                    if flussoElabMifValore is not null then
                    	...
                    	-- TBR
	                    if isTransElemAttiva=true or isTransElemCoToAttiva=true then
...
-- 		 	and	  date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(prov.validita_fine,dataFineVal))
        ...
        strMessaggio:='Lettura dati ordinativo numero='||mifOrdinativoIdRec.mif_ord_ord_numero
                       ...
                       ||'. Lettura dati configurazione per campo '||flussoElabMifElabRec.flusso_elab_mif_campo
                       ||' mifCountRec='||mifCountRec
                       ||' tipo flusso '||FLUSSO_MIF_ELAB_STATO_BENEF||'.';
        if flussoElabMifElabRec.flussoElabMifId is null then
            RAISE EXCEPTION ' Configurazione tag/campo non presente in archivio.';
        end if;
...
--					where cf.classif_id=programmaId
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true or isTransElemAttiva=true or isTransElemCoToAttiva=true then
 	 if flussoElabMifElabRec.flussoElabMifElab=true or isTransElemAttiva=true or isTransElemCoToAttiva=true then
      if flussoElabMifElabRec.flussoElabMifParam is not null or isTransElemAttiva=true or isTransElemCoToAttiva=true then
...
--		and	date_trunc('day',dataFineVal)<=date_trunc('day',coalesce(rEvento.validita_fine,dataFineVal))
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true or isTransElemAttiva=true  or isTransElemCoToAttiva=truethen
   	if flussoElabMifElabRec.flussoElabMifElab=true or isTransElemAttiva=true  or isTransElemCoToAttiva=true then
     if flussoElabMifElabRec.flussoElabMifParam is not null or isTransElemAttiva=true or isTransElemCoToAttiva=true then
...
--            where rclass.elem_id=mifOrdinativoIdRec.mif_ord_elem_id
 ...
 strMessaggio:='Lettura dati ordinativo numero='||mifOrdinativoIdRec.mif_ord_ord_numero
   ...
   -- 27.09.2016 Sofia COTO
   if flussoElabMifValore is not null then
    if isTransElemCoToAttiva=true then
   		    transElemCoTo:=flussoElabMifValore;
    end if;
    if isTransElemAttiva=false then
     flussoElabMifValore:=null;
    end if;
...
--            and   tipo.evento_tipo_code=flussoElabMifElabRec.flussoElabMifParam 15.02.2016 Sofia ABI36
  ...
  -- dati_a_disposizione_ente COTO -- 05.09.2016 Sofia
  -- <codben>
  flussoElabMifElabRec:=null;
  flussoElabMifValore:=null;
  mifCountRec:=FLUSSO_MIF_ELAB_DISP_BENEF_COTO;
  flussoElabMifElabRec:=mifFlussoElabMifArr[mifCountRec];
  strMessaggio:='Lettura dati ordinativo numero='||mifOrdinativoIdRec.mif_ord_ord_numero
                       ||' annoBilancio='||mifOrdinativoIdRec.mif_ord_anno_bil
                       ||' ord_id='||mifOrdinativoIdRec.mif_ord_ord_id
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
    	mifFlussoOrdDispeBenefRec.mif_ord_dispe_codben:=soggettoRec.soggetto_code;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
    	mifFlussoOrdDispeBenefRec.mif_ord_dispe_num_articolo:=bilElemRec.elem_code;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  end if;
  ...
  strMessaggio:='Lettura dati ordinativo numero='||mifOrdinativoIdRec.mif_ord_ord_numero
                       ...
                       ||' mifCountRec='||mifCountRec
                       ||' tipo flusso '||MANDMIF_TIPO||'.';
  if isTransElemCoToAttiva=true and transElemCoTo is not null then
  	mifFlussoOrdDispeBenefRec.mif_ord_dispe_trans_elem:=transElemCoTo;
  end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if flussoElabMifValore is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_pagamento:=null;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if flussoElabMifValore is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_cod_bilancio:=null;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if flussoElabMifValore is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_carte_corr:=null;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if codAccreRec.accredito_tipo_desc is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_forma_pagam:=codAccreRec.accredito_tipo_desc;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if mifFlussoOrdinativoRec.mif_ord_abi_benef is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_abi:=mifFlussoOrdinativoRec.mif_ord_abi_benef;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if mifFlussoOrdinativoRec.mif_ord_cab_benef is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_cab:=mifFlussoOrdinativoRec.mif_ord_cab_benef;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if flussoElabMifValore is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_cdc:=flussoElabMifValore;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if flussoElabMifValore is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_cod_bilancio:=flussoElabMifValore;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        ...
        if flussoElabMifValoreDesc is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_tipofin:=flussoElabMifValoreDesc;
            mifFlussoOrdDispeBenefRec.mif_ord_dispe_tipofin:=flussoElabMifValore;
        end if;
  ...
  mifCountRec:=mifCountRec+1;
  flussoElabMifElabRec:=mifFlussoElabMifArr[mifCountRec];
  if mifFlussoOrdDispeBenefRec.mif_ord_dispe_tipofin is not null then
    strMessaggio:='Lettura dati ordinativo numero='||mifOrdinativoIdRec.mif_ord_ord_numero
                       ||' annoBilancio='||mifOrdinativoIdRec.mif_ord_anno_bil
                       ||' ord_id='||mifOrdinativoIdRec.mif_ord_ord_id
                       ||' mif_ord_id='||mifOrdinativoIdRec.mif_ord_id
    ...
    else  mifFlussoOrdDispeBenefRec.mif_ord_dispe_tipofin:=null;
    end if;
  end if;

  -- <st_note_finanz_mutuo>
  flussoElabMifElabRec:=null;
  flussoElabMifValore:=null;
  mifCountRec:=mifCountRec+1;
  flussoElabMifElabRec:=mifFlussoElabMifArr[mifCountRec];
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if flussoElabMifValore is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_finanz_mutuo:=flussoElabMifValore;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if mifFlussoOrdinativoRec.mif_ord_anag_benef is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_ragsoc:=mifFlussoOrdinativoRec.mif_ord_anag_benef;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if mifFlussoOrdinativoRec.mif_ord_indir_benef is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_ragsoc_via:=mifFlussoOrdinativoRec.mif_ord_indir_benef;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if mifFlussoOrdinativoRec.mif_ord_cap_benef is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_ragsoc_cap:=mifFlussoOrdinativoRec.mif_ord_cap_benef;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if mifFlussoOrdinativoRec.mif_ord_localita_benef is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_ragsoc_comune:=mifFlussoOrdinativoRec.mif_ord_localita_benef;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if mifFlussoOrdinativoRec.mif_ord_prov_benef is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_ragsoc_prov:=mifFlussoOrdinativoRec.mif_ord_prov_benef;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if mifFlussoOrdinativoRec.mif_ord_codfisc_benef is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_ragsoc_codfisc:=mifFlussoOrdinativoRec.mif_ord_codfisc_benef;
        elsif  mifFlussoOrdinativoRec.mif_ord_partiva_benef is not null then
	         mifFlussoOrdDispeBenefRec.mif_ord_dispe_ragsoc_codfisc:=mifFlussoOrdinativoRec.mif_ord_partiva_benef;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  if flussoElabMifElabRec.flussoElabMifAttivo=true  then
   	if flussoElabMifElabRec.flussoElabMifElab=true  then
        if flussoElabMifValore is not null then
 	     	mifFlussoOrdDispeBenefRec.mif_ord_dispe_imp_ord_inc:=flussoElabMifValore;
        end if;
    else
		RAISE EXCEPTION ' Configurazione tag/campo  non elaborabile.';
    end if;
  ...
  (mif_ord_id,mif_ord_ord_id,
   ...
   mif_ord_dispe_codben, mif_ord_dispe_num_articolo,
   mif_ord_dispe_trans_elem,
   mif_ord_dispe_desc_pagamento,
   mif_ord_dispe_cod_bilancio,
   mif_ord_dispe_carte_corr,
   mif_ord_dispe_desc_forma_pagam,
   mif_ord_dispe_desc_abi,
   mif_ord_dispe_desc_cab,
   mif_ord_dispe_desc_cdc,
   mif_ord_dispe_desc_cod_bilancio,
   mif_ord_dispe_desc_tipofin,
   mif_ord_dispe_tipofin,
   mif_ord_dispe_finanz_mutuo,
   mif_ord_dispe_ragsoc,
   mif_ord_dispe_ragsoc_via,
   mif_ord_dispe_ragsoc_cap,
   mif_ord_dispe_ragsoc_comune,
   mif_ord_dispe_ragsoc_prov,
   mif_ord_dispe_ragsoc_codfisc,
   mif_ord_dispe_imp_ord_inc,
  ...
  (mifOrdSpesaId,mifOrdinativoIdRec.mif_ord_ord_id,
   ...
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_codben,  -- 05.09.2016 Sofi COTO
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_num_articolo,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_trans_elem,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_pagamento,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_cod_bilancio,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_carte_corr,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_forma_pagam,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_abi,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_cab,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_cdc,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_cod_bilancio,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_desc_tipofin,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_tipofin,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_finanz_mutuo,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_ragsoc,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_ragsoc_via,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_ragsoc_cap,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_ragsoc_comune,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_ragsoc_prov,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_ragsoc_codfisc,
   mifFlussoOrdDispeBenefRec.mif_ord_dispe_imp_ord_inc,-- 05.09.2016 Sofi COTO
