...
*/
REATE OR REPLACE FUNCTION fnc_mif_ordinativo_quote_entrata(ordinativoId integer,
...
BEGIN
 ...
 ( select ts.ord_ts_id ordTsId, ts.ord_ts_code numeroQuota, ts.ord_ts_desc descriQuota,
    ...
    from siac_t_ordinativo_ts ts, siac_t_ordinativo_ts_det det, siac_r_ordinativo_ts_movgest_ts mov
    where ts.ord_id=ordinativoId
    and   det.ord_ts_id=ts.ord_ts_id
    and   det.ord_ts_det_tipo_id=ordinativoTsDetTipoId
    and   mov.ord_ts_id=ts.ord_ts_id
...
/*    select ts.movgest_id, ts.movgest_ts_code, ts.movgest_ts_desc, ts.movgest_ts_tipo_id
    ...
    where ts.movgest_ts_id=ordinativoTsRec.movgestTsId; 18.02.2016 Sofia */
...
/*    select m.movgest_anno::varchar , m.movgest_numero::varchar, m.movgest_desc
    ...
    where m.movgest_id=movGestId; 18.02.2016 Sofia */
...
/*    if movGestTsTipoId=movgestTsTipoSubId then
    ...
    strMessaggio:='Lettura quote ordinativo.Quota numero='||ordinativoTsRec.numeroQuota
    			   ||'.Lettura Atto Amm legato al movimento dei gestione.';
  ...
  /* strMessaggio:='Lettura quote ordinativo.Quota numero='||ordinativoTsRec.numeroQuota
   ...
   if numeroSubMovGest is not null then
   ...
   if numeroAttoAmm is not null then
        ...
        if sacAttoAmm is not null then
	        movgestAttoAmmQuota := movgestAttoAmmQuota ||'-'||sacAttoAmm;
        end if;
