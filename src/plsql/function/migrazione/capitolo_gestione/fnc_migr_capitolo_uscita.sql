...
*/
CREATE OR REPLACE FUNCTION fnc_migr_capitolo_uscita (tipoElab varchar,
...
$body$
DECLARE
    -- fnc_migr_capitolo_uscita --> function che effettua il caricamento dei capitoli di uscita ( elementi di bialncio)
    -- leggendo in tab migr_capitolo_uscita
    ...
    -- siac_r_migr_capitolo_uscita_bil_elem --> relazione tra elemento di bilancio e capitolo migrato
     -- richiama
	 -- fnc_migr_aggiorna_classif_cap
        -- per aggiornamento delle descrizioni dei classificatori caricati
        -- in migr_classif_capitolo
     -- fnc_inserisci_capitolo_uscita  per
        -- caricamento del capitolo di uscita, letto dalla migr_capitolo_uscita
    -- restituisce
	...
	migrCapitoloUscita  migr_capitolo_uscita%rowtype;
    migrAggClassifCap record;
    migrCapUsciRetIns record;

    countMigrCapUsc integer:=0;
    ...
    flagFondoSvalCredAttrId integer:=0;
    flagFunzDelAttrId integer:=0;
	flagTrasfOrgAttrId integer:=0;
...
BEGIN
	...
	strMessaggioFinale:='Migrazione capitoli uscita da '||idMin||' a '||idmax||'.';
    strMessaggio:='Lettura capitoli uscita migrati.';

	select COALESCE(count(*),0) into countMigrCapUsc
    from migr_capitolo_uscita ms
     where ms.ente_proprietario_id=enteProprietarioId and
           ms.anno_esercizio=annoBilancio and
           ms.tipo_capitolo=bilElemTipo and
           ms.fl_elab='N'
           and ms.migr_capusc_id>=idMin and ms.migr_capusc_id<=idMax;

	if COALESCE(countMigrCapUsc,0)=0 then
    ...
    begin
     ...
     strMessaggio:='Lettura Identificativo attr_id= '||FLAG_IMPEGNABILE||'.';
	 ...
	 strMessaggio:='Lettura Identificativo attr_id= '||FLAG_FONDO_SVAL_CRED||'.';
     select attrTipi.attr_id into strict flagFondoSvalCredAttrId
	 from siac_t_attr attrTipi
	 ...
	 strMessaggio:='Lettura Identificativo attr_id= '||FLAG_FUNZ_DEL||'.';
     select attrTipi.attr_id into strict flagFunzDelAttrId
	 from siac_t_attr attrTipi
	 ...
	 strMessaggio:='Lettura Identificativo attr_id= '||FLAG_TRASF_ORG||'.';
     select attrTipi.attr_id into strict flagTrasfOrgAttrId
	 from siac_t_attr attrTipi
    ...
    parPerInsertElemBil.flagFondoSvalCredAttrId:=flagFondoSvalCredAttrId;
    parPerInsertElemBil.flagFunzDelAttrId:=flagFunzDelAttrId;
    parPerInsertElemBil.flagTrasfOrgAttrId:=flagTrasfOrgAttrId;
    parPerInsertElemBil.noteCapAttrId:=noteCapAttrId;

-- 20.10.2015 eseguire solo per il primo blocco
    select ms.migr_capusc_id into v_migr_cap_id
      from migr_capitolo_uscita ms
      where ms.ente_proprietario_id=enteProprietarioId
      and ms.anno_esercizio=annoBilancio
      and ms.tipo_capitolo=bilElemTipo
      and ms.fl_elab='N'
      order by ms.migr_capusc_id limit 1;
    ...
    (select ms.*
     ...
     where ms.ente_proprietario_id=enteProprietarioId and
           ...
           ms.fl_elab='N'
           and ms.migr_capusc_id>=idMin and ms.migr_capusc_id<=idMax
     order by ms.migr_capusc_id)
    loop
		strMessaggio:='Inserimento capitolo uscita per migr_capusc_id= '
                               ||migrCapitoloUscita.migr_capusc_id||'.';
		select * into migrCapUsciRetIns
        from fnc_inserisci_capitolo_uscita (tipoElab,bilancioId,annoBilancio,bilElemTipo,
        								    parPerInsertElemBil,
                                            migrCapitoloUscita,
                                            enteProprietarioId,loginOperazione,dataElaborazione);
        if migrCapUsciRetIns.codiceRisultato=-1 then
         RAISE EXCEPTION ' % ', migrCapUsciRetIns.messaggioRisultato;
        end if;


         strMessaggio:='Inserimento siac_r_migr_capitolo_uscita_bil_elem per migr_capusc_id= '
                               ||migrCapitoloUscita.migr_capusc_id||'.';
	     insert into siac_r_migr_capitolo_uscita_bil_elem
	    (migr_capusc_id,elem_id,ente_proprietario_id,data_creazione)
	    values
	    (migrCapitoloUscita.migr_capusc_id,migrCapUsciRetIns.bilElemid,enteProprietarioId,statement_timestamp());

         numeroElementiInseriti:=numeroElementiInseriti+1;
    end loop;

	strMessaggio:='Set fl_elab in migr_capitolo_uscita.';
	update migr_capitolo_uscita
    set fl_elab = 'S'
    where ente_proprietario_id = enteProprietarioId
    and anno_esercizio=annoBilancio
    and tipo_capitolo=bilElemTipo
    and migr_capusc_id >= idMin and migr_capusc_id <= idMax
    and fl_elab = 'N'
    and migr_capusc_id in (select r.migr_capusc_id from siac_r_migr_capitolo_uscita_bil_elem r where r.ente_proprietario_id=enteProprietarioId);


   RAISE NOTICE 'NumeroImpegniInseriti %', numeroElementiInseriti;


   messaggioRisultato:=strMessaggioFinale||'Inseriti '||numeroElementiInseriti||' capitoli uscita.';
