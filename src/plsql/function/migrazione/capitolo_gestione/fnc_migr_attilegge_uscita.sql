...
*/
CREATE OR REPLACE FUNCTION fnc_migr_attilegge_uscita (enteProprietarioId integer,
...
$body$
DECLARE
    -- fnc_migr_attilegge_uscita --> function che effettua il caricamento dei atti di legge
    --                               relazionandoli ai capitoli di uscita
    -- leggendo in tab migr_attilegge_uscita
    ...
    -- effettua inserimento di
     -- siac_r_bil_elem_attilegge --> relazione tra capitolo di uscita e atto di legge
    -- restituisce
     ...
     -- numeroElementiInseriti valorizzato con
        -- -12 dati da migrare non presenti in migr_attilegge_uscita
        -- -1 errore
...
--	dataInizioVal:=annoBilancio||'-01-01';
	...
	strMessaggioFinale:='Migrazione atti legge uscita.';
    strMessaggio:='Lettura atti legge uscita migrati.';


	select COALESCE(count(*),0) into countMigrAttiLeggeCap
    from migr_attilegge_uscita ms
    ...
    (select ms.*
     ...
     where ms.ente_proprietario_id=enteProprietarioId and
           ...
           ms.tipo_capitolo=bilElemTipo and
           ms.fl_elab='N'
     order by ms.migr_attilegge_usc_id)
    loop

        begin
        	strMessaggio:='Lettura capitolo per migr_attilegge_usc_id='||migrAttiLeggeCap.migr_attilegge_usc_id||'.';
...
--                  tipoCap.elem_tipo_code=bilElemTipo;
		...
		begin
		 ...
		 strMessaggio:='Lettura atto Legge migr_attilegge_usc_id='||migrAttiLeggeCap.migr_attilegge_usc_id||'.';
...
--               attoTipo.ente_proprietario_id=enteProprietarioId and
          ...
          exception
        	when no_data_found then
	        strMessaggio:='Inserimento atto Legge migr_attilegge_usc_id='||migrAttiLeggeCap.migr_attilegge_usc_id||'.';
	        ...
	        strMessaggio:='Inserimento atto Legge stato migr_attilegge_usc_id='||migrAttiLeggeCap.migr_attilegge_usc_id||'.';
...
--			            or stato.validita_fine is null));
        ...
        strMessaggio:='Inserimento siac_r_bil_elem_atto_legge per migr_attilegge_usc_id= '
                               ||migrAttiLeggeCap.migr_attilegge_usc_id||'.';
        ...
        returning attolegge_bil_elem_id into attoLeggeBilElemId;
			...
			strMessaggio:='Inserimento siac_r_migr_attilegge_usc per migr_attilegge_usc_id= '
                               ||migrAttiLeggeCap.migr_attilegge_usc_id||'.';
		 insert into  siac_r_migr_attilegge_usc
         (migr_attilegge_usc_id,attolegge_bil_elem_id,
		  tipo_bil_elem,data_creazione,ente_proprietario_id)
          values
          (migrAttiLeggeCap.migr_attilegge_usc_id,attoLeggeBilElemId,
	...
	-- 21.10.2015 dani
    update migr_attilegge_uscita ms
    set fl_elab='S'
    where ms.ente_proprietario_id=enteProprietarioId and
          ms.anno_esercizio=annoBilancio and
          ms.tipo_capitolo=bilElemTipo and
   ...
   messaggioRisultato:=strMessaggioFinale||'Inseriti '||numInseriti||' atti legge cap. uscita .';
