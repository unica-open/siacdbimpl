...
*/
create or replace function fnc_migr_ordinativo_ts_entrata(
...
BEGIN
    ...
    begin
        ...
        select distinct 1 into strict countRecordDaMigrare from migr_ordinativo_entrata_ts m
         where m.ente_proprietario_id=enteproprietarioid
           and m.fl_elab='N'
           and m.migr_ordinativo_entrata_ts_id >= idmin
           and m.migr_ordinativo_entrata_ts_id <= idmax;
    ...
    --INIZIO DEL LOOP---------
    --------------------------
    strMessaggio:='Lettura record migr_ordinativo_entrata_ts.';
    
    for migrRecord IN
        (
         select
           migr_ordinativo_entrata_ts_id 
          ,ordinativo_ts_id    
          ,ordinativo_id        
          ,anno_esercizio        
          ,numero_ordinativo
          ...
          ,data_creazione 
         from
             migr_ordinativo_entrata_ts m
         where m.ente_proprietario_id=enteproprietarioid
               and m.fl_elab='N'
               and m.migr_ordinativo_entrata_ts_id >= idmin
               and m.migr_ordinativo_entrata_ts_id <= idmax
         order by m.migr_ordinativo_entrata_ts_id
        )
    LOOP
        ...
        if migrRecord.importo_attuale = 0 then
          strMessaggioScarto := 'ordinativo quota (reversale) non migrata  x importo = 0 '||migrRecord.anno_esercizio||'/'||migrRecord.numero_ordinativo||'/'||migrRecord.quota_ordinativo||'.';
        else
                 
            strMessaggio:='Ricerca Id dell''accertamento relativo all''ordinativo quota (reversale): '||migrRecord.anno_esercizio||'/'||migrRecord.numero_ordinativo||'/'||migrRecord.quota_ordinativo||'.';
    
            if migrRecord.numero_subaccertamento = 0 then
            ...
            select movt.movgest_ts_id
             ...
             where mov.movgest_anno = migrRecord.anno_accertamento
               and mov.movgest_numero = migrRecord.numero_accertamento
            ...
            if v_movgest_ts_id is null then
                select 1 
                  into v_scarto 
                  from migr_ordinativo_entrata_ts_scarto 
                 where migr_ordinativo_entrata_ts_id=migrRecord.migr_ordinativo_entrata_ts_id;

                if v_scarto is null then
                    strMessaggioScarto := 'quota ordinativo non inserita per accertamento non migrato. Ordinativo : '||migrRecord.numero_ordinativo||'/'||migrRecord.anno_accertamento||'/'||migrRecord.numero_accertamento||'/'||migrRecord.numero_subaccertamento;
                end if;
            end if;
            ...
            select ordin.ord_id
             ...
             where ordin.ord_anno = migrRecord.anno_esercizio
               ...
               and ordin.ord_tipo_id = v_ord_ts_det_tipo_id_a
        ...
        if strMessaggioScarto is null then
            ...
            strMessaggio:='Inserimento dell''ordinativo quota (Reversale) : '||migrRecord.anno_esercizio||'/'||migrRecord.numero_ordinativo||'/'||migrRecord.quota_ordinativo||'.';
            ...
            if v_ord_ts_id is null then
                strMessaggioScarto := 'quota ordinativo non inserita per errore. Ordinativo : '||migrRecord.numero_ordinativo||'/'||migrRecord.anno_accertamento||'/'||migrRecord.numero_accertamento||'/'||migrRecord.numero_subaccertamento;
            else
                ...
                insert into siac_r_migr_ordinativo_ts_entrata_ordinativo (
                    migr_ordinativo_entrata_ts_id
                    ,ord_ts_id
                    ,data_creazione
                    ,ente_proprietario_id
                ...
                update migr_ordinativo_entrata_ts 
                   set fl_elab='S'
                 where ente_proprietario_id=enteProprietarioId
                   and migr_ordinativo_entrata_ts_id = migrRecord.migr_ordinativo_entrata_ts_id;

            end if;
        end if;

        if strMessaggioScarto is not null then
            insert into migr_ordinativo_entrata_ts_scarto (
                migr_ordinativo_entrata_ts_id,
                numero_ordinativo,
                anno_esercizio,
                motivo_scarto, 
                ...
                migrRecord.migr_ordinativo_entrata_ts_id,
    ...
    messaggioRisultato:=strMessaggioFinale||'Inserite '||countRecordInseriti||' quote ordinativi entrata.';
