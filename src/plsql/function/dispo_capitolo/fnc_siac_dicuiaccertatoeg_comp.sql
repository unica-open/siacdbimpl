...
*/
CREATE OR REPLACE FUNCTION fnc_siac_dicuiaccertatoeg_comp (
  id_in integer,
...
if coalesce(anno_in,NVL_STR)!=NVL_STR then
    ...
    strMessaggio:='Calcolo accertato competenza elem_id='||id_in||'.Anno='||anno_in||'.';
ELSE
	begin
     strMessaggio:='Calcolo accertato competenza elem_id='||id_in||'.Calcolo anni triennale.';
