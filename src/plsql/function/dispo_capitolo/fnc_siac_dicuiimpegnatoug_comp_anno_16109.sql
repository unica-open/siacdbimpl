...
*/
CREATE OR REPLACE FUNCTION fnc_siac_dicuiimpegnatoug_comp_anno_16109 (
  id_in integer,
  anno_in varchar
)
RETURNS TABLE (
...
importoCurAttuale1 numeric:=0;
importoCurAttuale numeric:=0;

importoModifNeg  numeric:=0;
importoModifNeg1  numeric:=0;
...
select tb.importo into importoCurAttuale1
 from (
  select
      coalesce(sum(e.movgest_ts_det_importo),0)  importo
      , c.movgest_ts_tipo_id
    from
      ...
      and c.movgest_id=b.movgest_id
      and d.movgest_ts_id=c.movgest_ts_id
      and ra.movgest_ts_id=c.movgest_ts_id
      and ra.attoamm_id=16109 -- determina 1246/2017 di EA1
      and d.validita_fine is null
      and e.movgest_ts_id=c.movgest_ts_id
	and a.data_cancellazione is null
    and b.data_cancellazione is null
...
/*select tb.importo into importoCurAttuale from (
 ...
 if importoCurAttuale1 is null then importoCurAttuale1:=0; end if;

 if importoCurAttuale>importoCurAttuale1 then
	 importoCurAttuale:=importoCurAttuale-importoCurAttuale1;
 else importoCurAttuale:=0;
 end if;
...
select tb.importo into importoModifNeg1
 from
 (
 	select coalesce(sum(moddet.movgest_ts_det_importo),0) importo , ts.movgest_ts_tipo_id
	from siac_r_movgest_bil_elem rbil, siac_t_movgest mov,siac_t_movgest_ts ts,
  ...
  order by tipo.movgest_ts_tipo_code desc
  limit 1;
  if importoModifNeg1 is null then importoModifNeg1:=0; end if;

  if abs(importoModifNeg)>abs(importoModifNeg1) then
  	importoModifNeg:=importoModifNeg-importoModifNeg1;
  else importoModifNeg:=0;
  end if;
 end if;
