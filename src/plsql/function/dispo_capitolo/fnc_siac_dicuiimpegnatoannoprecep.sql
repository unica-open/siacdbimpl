...
*/
CREATE OR REPLACE FUNCTION fnc_siac_dicuiimpegnatoannoprecep (id_in integer )
RETURNS  numeric AS
$body$
DECLARE
...
BEGIN
 ...
 strMessaggio:='Calcolo accertato competenza anno prec elem_id='||id_in||'.Controllo parametri.';
 ...
 strMessaggio:='Calcolo accertato competenza UP anno prec elem_id='||id_in||'.';


 strMessaggio:='Calcolo accertato competenza UP anno prec.Calcolo bilancioId e elem_tipo_code per elem_id='||id_in||'.';
 ...
 strMessaggio:='Calcolo accertato competenza EP anno prec.Calcolo bilancioId per elem_id equivalente anno prec'
 ...
 strMessaggio:='Calcolo accertato competenza EP anno prec.Calcolo elem_id equivalente per bilancioId='||bilIdElemGestEq
              ||' per ente='||enteProprietarioId||' per elem_id='||id_in||'.';
...
if NOT FOUND THEN
 ...
 strMessaggio:='Calcolo accertato competenza anno prec elem_id='||id_in||'.'
