...
*/
CREATE OR REPLACE FUNCTION fnc_siac_disponibilitapagareug (
  id_in integer
)
RETURNS numeric AS
$body$
...
BEGIN
    ...
    strMessaggio:='Calcolo disponibilita cassa  elem_id='||id_in||
                  'Lettura totale pagato.';
    totPagato:=fnc_siac_totalepagatoug(id_in);

    dispCassa:=stanzEffettivoRec.stanzEffettivoCassa-totPagato;
