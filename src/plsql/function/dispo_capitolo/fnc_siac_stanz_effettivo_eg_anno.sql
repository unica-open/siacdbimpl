...
*/
CREATE OR REPLACE FUNCTION siac.fnc_siac_stanz_effettivo_eg_anno (
...
BEGIN
 ...
 if id_in is not null and id_in!=0 then
        ...
        and   tipoBilElem.elem_tipo_code = TIPO_CAP_EG
...
/*  04.07.2016 Sofia ID-INC000001114035 non considerare le variazioni di gestione*/
   ...
   -- raise '% varImpGestione=% stanziamentoPrev=% stanziamento=%' ,
  --        strMessaggio,varImpGestione,stanziamentoPrev,stanziamento;
 end if;
...
-- 		       tipoStatoVar.variazione_stato_tipo_code in (STATO_VAR_G,STATO_VAR_C,STATO_VAR_P) and -- 26072016 Sofia JIRA-SIAC-3887
 ...
 strMessaggio:='Stanziamento effettivo elem_id='||bilElemId||' bilFaseOperativa '||bilFaseOperativa||'.';
-- raise '% stanziamentoPrev=% stanziamento=%' ,strMessaggio,stanziamentoPrev,stanziamento;
 case
