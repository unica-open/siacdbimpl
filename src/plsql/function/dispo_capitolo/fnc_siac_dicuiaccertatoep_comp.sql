...
*/
CREATE OR REPLACE FUNCTION fnc_siac_dicuiaccertatoep_comp (
  id_in integer,
...
BEGIN
 ...
 begin
    ...
    if coalesce(elemTipoCode,NVL_STR)!=TIPO_CAP_EP then
...
if coalesce(anno_in,NVL_STR)!=NVL_STR then
    ...
    strMessaggio:='Calcolo accertato competenza EP elem_id='||elemIdGestEq||'.Anno='||anno_in||'.';
ELSE
     strMessaggio:='Calcolo accertato competenza EP elem_id='||elemIdGestEq||'.Calcolo sul triennale.';
...
for movGestRec in
 ...
 where  bilElem.elem_id = elemIdGestEq and
        ...
        bilElemTipo.elem_tipo_code=TIPO_CAP_EG and
...
loop
	...
	diCuiAccertato:=movGestRec.importoAttuale;

    totAccertato:=totAccertato+movGestRec.importoAttuale;

	return next;
end loop;

if totAccertato!=0 THEN
	return;
else
    return next;
end if;
