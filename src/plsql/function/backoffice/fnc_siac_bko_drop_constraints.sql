...
*/
CREATE OR REPLACE FUNCTION siac.fnc_siac_bko_drop_constraints (
)
RETURNS TABLE (
...
delete from bko_t_constraints_create_drop;

insert into bko_t_constraints_create_drop (c_operation,c_command,c_type,c_name,c_table_name)
SELECT 
'ADD CONSTRAINT',
...
for fnc_rec in 
select c_command from bko_t_constraints_create_drop where c_operation='DROP CONSTRAINT' order by c_order 
loop
