...
*/
CREATE OR REPLACE FUNCTION siac.fnc_siac_bko_configura_report (
)
RETURNS void AS
$body$
DECLARE
...
where --ente_proprietario_id=1 and 
data_cancellazione is null 
--and rep_codice in ('BILR001','BILR005')
loop
...
where 
re.rep_codice=ko.rep_codice
and ri.repimp_codice=ko.repimp_codice
and ri.ente_proprietario_id=re.ente_proprietario_id
and not exists 
(select 1 from siac_r_report_importi ii where ii.rep_id=re.rep_id and 
ii.repimp_id=ri.repimp_id
);
