...
*/
CREATE OR REPLACE FUNCTION siac.fnc_siac_td_aggiorna_pdcfin_movimento (
  anno_bil varchar,
...
DECLARE
    ...
    mov_id_new siac_t_movgest_ts.movgest_ts_id%type;
...
BEGIN
    ...
    select b.movgest_ts_id into mov_id_new 
    ...
    if classif_id_new is not null and 
    	mov_id_new is not null then 
          ...
          UPDATE 
            siac.siac_r_movgest_class 
          SET 
            validita_fine = now(),
            data_cancellazione = now(),
            login_operazione = login_operazione || ' - ' || numero_incident
          WHERE 
            movgest_classif_id in (
               select x.movgest_classif_id from siac_r_movgest_class x,
               siac_t_class y, siac_d_class_tipo z
               where x.movgest_ts_id=mov_id_new
