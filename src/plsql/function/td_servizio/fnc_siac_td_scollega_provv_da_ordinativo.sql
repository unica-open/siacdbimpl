...
*/
CREATE OR REPLACE FUNCTION siac.fnc_siac_td_scollega_provv_da_ordinativo (
  anno_bil varchar,
...
BEGIN
    v_messaggiorisultato :='Errore';
      ...
      if ord_id_new is not null and prov_id_new is not null 
         and ord_provc_id_new is not null then 
           
        UPDATE 
          siac.siac_r_ordinativo_prov_cassa 
        SET 
          validita_fine = data_modifica_new,
          data_cancellazione = data_modifica_new,
          login_operazione = login_operazione || ' - ' || login_operazione_new
        WHERE 
          ord_provc_id = ord_provc_id_new
        ;
    ...
    if ord_id_new is null or prov_id_new is null or ord_provc_id_new is null then 
