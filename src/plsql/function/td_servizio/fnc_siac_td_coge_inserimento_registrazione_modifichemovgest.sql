...
*/
CREATE OR REPLACE FUNCTION siac.fnc_siac_td_coge_inserimento_registrazione_modifichemovgest (
  movgest_anno_in integer,
  movgest_numero_in integer,
  movgest_tipo_code_in varchar,
  modifica_num_in INTEGER,
...
if movgest_tipo_code_in ='A' then 
 evento_code_new:='MAC-INS-I';
elsif movgest_tipo_code_in ='I' then 
evento_code_new:='MIM-INS-I';
else
--errore
...
select 
l.mod_id, e.classif_id,a.bil_id,g.ambito_id into
mod_id_new,classif_id_new, bil_id_new,ambito_id_new
from 
siac_t_movgest a,
...
--
siac_t_modifica l,
siac_r_modifica_stato m, 
siac_d_modifica_stato n,
siac_t_movgest_ts_det_mod o
where a.ente_proprietario_id=ente_proprietario_id_in
and a.movgest_anno=movgest_anno_in 
and a.movgest_numero=movgest_numero_in
...
and i.anno=bil_anno
and o.movgest_ts_id=c.movgest_ts_id
and l.mod_id=m.mod_id
and l.mod_num=modifica_num_in
and now() BETWEEN m.validita_inizio and coalesce (m.validita_fine,now())
and o.mod_stato_r_id=m.mod_stato_r_id
and m.mod_stato_id=n.mod_stato_id
and n.mod_stato_code='V'
and a.data_cancellazione is NULL
...
select regmovfin_id_new,a.evento_id, mod_id_new,now(),a.ente_proprietario_id,login_operazione_in
