...
*/
-- 17.11.2016 Sofia Impegni residui , senza considerare le liquidazioni
-- importoRes=impegnato-pagato
-- solo impegni D,N con importoRes>0
-- sono da eseguire prima di accertamenti residui

CREATE OR REPLACE FUNCTION fnc_fasi_bil_gest_apertura_imp_puntuale
(
...
BEGIN
	...
	strMessaggioFinale:='Apertura bilancio gestione per Anno bilancio='||annoBilancio::varchar||
                        '. Ribaltamento impegni residui.';

	strmessaggio:='Calcola impegni residui da creare.';
    select * into strRec
    from fnc_fasi_bil_gest_apertura_imp_popola_puntuale
    (enteproprietarioid,
     annobilancio,
     loginoperazione,
	 dataelaborazione
