...
*/
CREATE OR REPLACE FUNCTION fnc_fasi_bil_gest_allinea_res_acc
(
...
BEGIN
	...
	strMessaggioFinale:='Apertura bilancio gestione per Anno bilancio='||annoBilancio::varchar||
                        '. Allineamento accertamenti residui.';
    ...
    select * into strRec
	from fnc_fasi_bil_gest_apertura_acc_popola
    (enteproprietarioid,
     annobilancio,
     loginoperazione,
	 dataelaborazione
    ...
    if codiceRisultato=0 then
     ...
     strMessaggio:='Allineamento accertamenti residui : importo_res diverso da attuale.Allinea attuale';
	 codResult:=null;
	 insert into fase_bil_t_elaborazione_log
     (fase_bil_elab_id,fase_bil_elab_log_operazione,
      validita_inizio, login_operazione, ente_proprietario_id
     ...
     strMessaggio:='Allineamento accertamenti residui : importo_res diverso da attuale.Azzera attuale';
	 codResult:=null;
	 insert into fase_bil_t_elaborazione_log
     (fase_bil_elab_id,fase_bil_elab_log_operazione,
      validita_inizio, login_operazione, ente_proprietario_id
...
--       	    login_operazione=det.login_operazione||'-'||loginOperazione||'-AZZERA-ATT-'||faseBilElabId::varchar
     ...
     strMessaggio:='Allineamento accertamenti residui : importo_res diverso da attuale.Allinea-azzera iniziale';
	 codResult:=null;
	 insert into fase_bil_t_elaborazione_log
     (fase_bil_elab_id,fase_bil_elab_log_operazione,
      validita_inizio, login_operazione, ente_proprietario_id
...
strMessaggio:='Allineamento accertamenti residui : importo_res diverso da attuale.Allinea-azzera utilizzabile';
	 codResult:=null;
	 insert into fase_bil_t_elaborazione_log
     (fase_bil_elab_id,fase_bil_elab_log_operazione,
      validita_inizio, login_operazione, ente_proprietario_id
	 ...
	 set    movgest_ts_det_importo=QUERY.importo_attuale,
            data_modifica=clock_timestamp(),
--            login_operazione=det.login_operazione||'-'||loginOperazione||'-ALLINEA-AZZERA-UTI-'||faseBilElabId::varchar
            login_operazione=loginOperazione||'-ALLINEA-AZZERA-UTI-'||faseBilElabId::varchar
     ...
     strMessaggio:='Allineamento accertamenti residui : inserimento nuovi residui.Aggiorna fase_bil_t_gest_apertura_acc.';
     codResult:=null;
	 insert into fase_bil_t_elaborazione_log
     (fase_bil_elab_id,fase_bil_elab_log_operazione,
      validita_inizio, login_operazione, ente_proprietario_id
	...
	select residui2017.anno_impegno,
	       ...
	       residui2017.movgest_orig_ts_id,
    	   residui2017.fl_elab,
	       residui2017.fase_bil_gest_ape_acc_id
	 from residui2017
     	  left join  residui2018 on -- non passati da aggiornare 2018
          (residui2017.anno_impegno=residui2018.anno_impegno
	 and   residui2017.numero_impegno=residui2018.numero_impegno
	 and   residui2017.numero_subimpegno=residui2018.numero_subimpegno)
	 where residui2018.importo_attuale is null
	 and   residui2017.fase_bil_gest_ape_acc_id=fase.fase_bil_gest_ape_acc_id
	);
     ...
     if coalesce(codResult)!=0 then
	  ...
	  strMessaggio:='Allineamento accertamenti residui : inserimento nuovi residui.Esecuzione fnc_fasi_bil_gest_apertura_acc_elabora.';
      codResult:=null;
	  insert into fase_bil_t_elaborazione_log
      (fase_bil_elab_id,fase_bil_elab_log_operazione,
       validita_inizio, login_operazione, ente_proprietario_id
 	...
 	if codiceRisultato=0 then
     strMessaggio:='Allineamento accertamenti residui : calcolo totale residuo aggiornato.';
     codResult:=null;
	 insert into fase_bil_t_elaborazione_log
     (fase_bil_elab_id,fase_bil_elab_log_operazione,
      validita_inizio, login_operazione, ente_proprietario_id
...
--	 group by  tipots.movgest_ts_tipo_code
	 ...
	 strMessaggio:='Allineamento accertamenti residui : calcolo totale residuo ricalcolato.';
     codResult:=null;
	 insert into fase_bil_t_elaborazione_log
     (fase_bil_elab_id,fase_bil_elab_log_operazione,
      validita_inizio, login_operazione, ente_proprietario_id
    ...
    if codiceRisultato=0 then
       ...
       set fase_bil_elab_esito='KO',
           fase_bil_elab_esito_msg='ELABORAZIONE FASE BILANCIO '||APE_GEST_ACC_RES||'TERMINATA CON ERRORE.',
           validita_fine=clock_timestamp()
       where fase_bil_elab_id=faseBilElabId;
	end if;
