...
*/
CREATE OR REPLACE FUNCTION fnc_fasi_bil_gest_apertura_liq_popola_imp(
...
BEGIN
	...
	strMessaggioFinale:='Apertura bilancio gestione.Ribaltamento impegni residui per liquidazioni residue da Gestione precedente. Anno bilancio='||annoBilancio::varchar||'. POPOLA.';
    ...
    if codResult is null then
     ...
     -- impegni con pagamenti, se hanno pagamenti hanno anche liquidazioni, impegnato del padre - pagato di se stesso + tutti subimpegni
     -- aggiornare dopo movgest_orig_ts_id di impegno padre
     -- imp_orig_importo
     ...
     (fase_bil_elab_id,
      ...
      (select faseBilElabId,
              ...
              detm.movgest_ts_det_importo, -- imp_orig_importo  da aggiornare dopo
              sum(det.ord_ts_det_importo), -- imp_orig_pagato
              mov.bil_id,
              re.elem_id,
              bilancioId,
              ...
              loginOperazione,
              clock_timestamp()
       from siac_t_bil bil,siac_t_movgest mov,siac_t_movgest_ts movts,siac_d_movgest_tipo tipo,
...
/*       and   exists (select 1 from fase_bil_t_gest_apertura_liq faseliq
                     where faseliq.fase_bil_elab_id=faseBilElabId
                     and   faseliq.fl_elab='N'
                     and   faseliq.movgest_orig_id=mov.movgest_id
                     and   faseliq.movgest_orig_ts_id=movts.movgest_ts_id
                     and   faseliq.data_cancellazione is null
                     and   faseliq.validita_fine is null
                     order by faseliq.fase_bil_gest_ape_liq_id
                     limit 1)*/
...
--                detm.movgest_ts_det_importo,
     ...
     codResult:=null;
     strMessaggio:='Inserimento fase_bil_t_gest_apertura_liq_imp impegni senza dettaglio di pagamento con liquidazioni valide - INIZIO.';
	 insert into fase_bil_t_elaborazione_log
     (fase_bil_elab_id,fase_bil_elab_log_operazione,
      validita_inizio, login_operazione, ente_proprietario_id
     )
  ...
  --    imp_orig_importo,
      ...
      (select faseBilElabId,
              MOVGEST_TS_T_TIPO,
              mov.movgest_id,
--              movts.movgest_ts_id, vedi sopra inserimento con dettaglio pagamento
              sum(detm.movgest_ts_det_importo), -- imp_importo
--              detm.movgest_ts_det_importo, -- imp_orig_importo
       ...
       from siac_t_bil bil,siac_t_movgest mov,siac_t_movgest_ts movts,siac_d_movgest_tipo tipo,
            siac_t_movgest_ts_det detm, siac_d_movgest_ts_det_tipo tipodetm,
            siac_r_liquidazione_movgest rliq, siac_r_movgest_bil_elem re,
            siac_t_liquidazione l, siac_r_liquidazione_stato rs, siac_d_liquidazione_stato stato
       where bil.bil_id=bilancioPrecId
       and   mov.bil_id=bil.bil_id
       and   mov.movgest_tipo_id=movGestTipoId
       and   re.movgest_id=mov.movgest_id
       ...
       and   not exists (select 1
                         ...
                         where rliqord.liq_id=rliq.liq_id
					       and ts.ord_ts_id=rliqord.sord_id
					       and ord.ord_id=ts.ord_id
					       and rord.ord_id=ord.ord_id
					       and rord.ord_stato_id!=ordStatoAId
...
/*       and   exists (select 1 from fase_bil_t_gest_apertura_liq faseliq
     ...
     codResult:=null;
     strMessaggio:='Inserimento fase_bil_t_gest_apertura_liq_imp su impegni senza dettaglio di pagamento con liquidazioni valide  - FINE.';
	 insert into fase_bil_t_elaborazione_log
     (fase_bil_elab_id,fase_bil_elab_log_operazione,
      validita_inizio, login_operazione, ente_proprietario_id
     )
     ...
     codResult:=null;
     strMessaggio:='Inserimento fase_bil_t_gest_apertura_liq_imp impegni senza dettaglio di pagamento con subimpegni - INIZIO.';
	 insert into fase_bil_t_elaborazione_log
     (fase_bil_elab_id,fase_bil_elab_log_operazione,
      validita_inizio, login_operazione, ente_proprietario_id
     )
     ...
     (fase_bil_elab_id,
      ...
      (select faseBilElabId,
       ...
       and   re.data_cancellazione is null
       and   re.validita_fine is null
       and   exists (select 1 from fase_bil_t_gest_apertura_liq faseliq -- esiste in tab liq da elaborare
                     where faseliq.fase_bil_elab_id=faseBilElabId
                     and   faseliq.fl_elab='N'
                     and   faseliq.movgest_orig_id=mov.movgest_id
                     and   faseliq.movgest_orig_ts_id=movts.movgest_ts_id
                     ...
                     order by faseliq.fase_bil_gest_ape_liq_id
                     limit 1)
	   and   exists (select 1 from fase_bil_t_gest_apertura_liq faseliq, siac_t_movgest_ts tssub -- esiste in tab lib da elaborare anche come sub
                     where faseliq.fase_bil_elab_id=faseBilElabId
                     and   faseliq.fl_elab='N'
                     and   faseliq.movgest_orig_id=mov.movgest_id
                     and   tssub.movgest_id=mov.movgest_id
                     and   tssub.movgest_ts_tipo_id=movGestTsSTipoId
                     and   faseliq.movgest_orig_ts_id=tssub.movgest_ts_id
                     and   faseliq.data_cancellazione is null
                     and   faseliq.validita_fine is null
                     and   tssub.data_cancellazione is null
                     and   tssub.validita_fine is null
                     order by faseliq.fase_bil_gest_ape_liq_id
                     limit 1)
	   and not exists (select 1 from fase_bil_t_gest_apertura_liq_imp fase1 -- non ancora inserito in tab imp
                       where fase1.fase_bil_elab_id=faseBilElabId
                       and   fase1.fl_elab='N'
                       and   fase1.movgest_orig_id=mov.movgest_id
                       and   fase1.movgest_orig_ts_id=movts.movgest_ts_id
                       and   fase1.data_cancellazione is null
                       and   fase1.validita_fine is NULL
                       order by fase1.fase_bil_gest_ape_liq_imp_id limit 1 )
       order by mov.movgest_anno::integer,mov.movgest_numero::integer
      );
     ...
     codResult:=null;
     strMessaggio:='Inserimento fase_bil_t_gest_apertura_liq_imp su impegni senza dettaglio di pagamento con subimpegni - FINE.';
	 insert into fase_bil_t_elaborazione_log
     (fase_bil_elab_id,fase_bil_elab_log_operazione,
      validita_inizio, login_operazione, ente_proprietario_id
     )
     ...
     --- copiare qui le stess insert per subimpegni
     codResult:=null;
