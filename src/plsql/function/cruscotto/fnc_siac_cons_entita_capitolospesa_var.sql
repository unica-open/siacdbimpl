...
*/
﻿CREATE OR REPLACE FUNCTION siac.fnc_siac_cons_entita_capitolospesa_var (
...
(with
 ...
 (select e.elem_id,detvar.variazione_stato_id,
         coalesce(sum(detvar.elem_det_importo),0) stanziamento_var
 ...
 varBozzaRes as
 (select e.elem_id,detvar.variazione_stato_id,
         coalesce(sum(detvar.elem_det_importo),0) stanziamento_res_var
  from  siac_t_bil bil, siac_t_periodo per,siac_t_bil_elem e,
        siac_t_bil_elem_det_var detvar, siac_d_bil_elem_det_tipo tipodet
  where per.periodo_id=bil.periodo_id
  and   detvar.periodo_id=per.periodo_id
  ...
  varBozzaCassa as
  (select e.elem_id,detvar.variazione_stato_id,
  	      coalesce(sum(detvar.elem_det_importo),0) stanziamento_cassa_var
   from  siac_t_bil bil, siac_t_periodo per,siac_t_bil_elem e,
         siac_t_bil_elem_det_var detvar, siac_d_bil_elem_det_tipo tipodet
   where per.periodo_id=bil.periodo_id
   and   detvar.periodo_id=per.periodo_id
...
variazioni.stanziamento_var,
variazioni.stanziamento_res_var,
variazioni.stanziamento_cassa_var
