...
*/
CREATE OR REPLACE FUNCTION siac.fnc_siac_fpv_spesa_capitolo (
...
--trovo gli anni su cui ciclare = anni su cui ci sono impegni legati al programma con anno >= anno imput
--limite inferiore ciclo = anno input
 ...
 SELECT tb.anno,
  ...
  FROM (
         ...
         WHERE imp.user_code=v_user and NOT EXISTS (
                             ...
                             fpv.classif_id_missione = imp.classif_id_missione AND 
                             fpv.classif_id_programma = imp.classif_id_programma AND
                             fpv.classif_id_titolo = imp.classif_id_titolo AND
                             fpv.anno::text = imp.anno::text and 
                             fpv.elem_id = imp.elem_id
               )
         GROUP BY imp.anno,
                  imp.classif_id_missione,
                  imp.classif_code_missione,
...
raise notice 'elem_id_out = %', elem_id_out;

select elem_code, elem_desc, elem_code2, elem_desc2, elem_code3 
into elem_code_out, elem_desc_out,elem_code2_out,elem_desc2_out,elem_code3_out
from siac_t_bil_elem where elem_id=elem_id_out;

raise notice 'elem_id_out=%,elem_code_out=%,elem_desc_out=%,elem_code2_out=% ',elem_id_out,elem_code_out,elem_desc_out,elem_code2_out;   
return next;
end loop;
