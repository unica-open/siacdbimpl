...
-- SIAC-7593 - Sofia 30072020 - inizio
...
i.movgest_ts_importo> -- vincoli impegno FPV/AAM che non sono già stati aggiornati da on-line su mod.spesa
(
...
FROM
(
--  vincoli impegno accertamento che non sono già stati aggiornati da on-line su mod.spesa (A)
-- (A)
(
select coalesce(sum(rvinc.importo_delta),0) importo_delta
from siac_r_modifica_vincolo rvinc
