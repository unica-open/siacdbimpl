...
-- (E)	Per gli impegni nati da ROR ma sprovvisti di indicazioni sui vincoli
--      sarà necessario
--      provvedere all’analisi di quanto effettuato in ROR:
--      Fresco per differimenti contestuali entrata-spesa, FPV da ROR altrimenti
-- (F)	Impegni senza indicazioni di vincolo sarà necessario esaminare ogni caso,
--      valutandone le sue caratteristiche, come ad esempio il provvedimento di assunzione

/*
for each (impegno in elencoImpegni) {
if (impegno.vincolo == accertamento) {
if (accertamento.anno == impegno.anno)
Componente = “Fresco”;
elif (accertamento.anno < impegno.anno) {
If (impegno.daRiaccertamento == true)
	Componente = FPV-A-ROR;
elif (impegno.daRiaccertamento = false)
Componente = FPV-A-GE;
}
} elif (impegno.vincolo == FPV) {
If (impegno.daRiaccertamento == true)
	Componente = FPV-A-ROR;
elif (impegno.daRiaccertamento = false)
Componente = FPV-A-GE;
} elif (impegno.vincolo == AAM)
Comoponente = Avanzo;
}
}
*/

-- (A)	Verrà utilizzata la componente Fresco per gli impegni vincolati
--      ad accertamenti della medesima annualità.

select imp.movgest_anno, imp.movgest_numero,imp.movgest_subnumero,
       acc.movgest_anno, acc.movgest_numero,acc.movgest_subnumero,
...
and   imp.movgest_anno=acc.movgest_anno
...
from impegno
  ...
  left join  siac_d_bil_elem_det_comp_tipo comp
    ...
    and comp.elem_det_comp_tipo_desc=query.componente_impegno_tipo
    and comp.data_cancellazione is  null )
  left join siac_r_movgest_ts rvinc
  on ( rvinc.movgest_ts_b_Id=query.movgest_ts_id
  and  rvinc.movgest_ts_r_id!=query.movgest_ts_r_id
  and  rvinc.data_cancellazione is null
  and  rvinc.validita_fine is null )
order by 1,2,3,4;
...
--where query_ext.elem_det_comp_tipo_desc is not null
order by 1,2,3,4;
