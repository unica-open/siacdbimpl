...
and   e.elem_tipo_id=tipo.elem_tipo_id
and   anno.bil_id=e.bil_id
and   anno.anno_bilancio in (2021)
...
from siac_t_bil_elem e,siac_d_bil_elem_tipo tipo,
     ...
     siac_t_periodo per
where tipo.ente_proprietario_id=2
and   tipo.elem_tipo_code in ('CAP-UG','CAP-UG')
and   e.elem_tipo_id=tipo.elem_tipo_id
and   anno.bil_id=e.bil_id
and   anno.anno_bilancio=2021
and   det.elem_id=e.elem_id
...
from cap_ug ug
     left join
     ( select dvar.elem_det_comp_id , count(*) var_count from siac_t_bil_elem_det_var_comp dvar
       where dvar.ente_proprietario_id=2 and dvar.data_cancellazione is null
       group by dvar.elem_det_comp_id ) ug_var
       on (ug_var.elem_det_comp_id=ug.elem_det_comp_id  )
     left join
     ( select rmov.elem_id, rmov.elem_det_comp_tipo_id , count(*) imp_count from siac_r_movgest_bil_elem rmov
       where rmov.ente_proprietario_id=2 and rmov.data_cancellazione is null
       group by rmov.elem_id, rmov.elem_det_comp_tipo_id ) ug_rmov
       on (ug_rmov.elem_id=ug.elem_id and ug_rmov.elem_det_comp_tipo_id=ug.elem_det_comp_tipo_id  )
where
not exists
...
-- ci sono in gestione e non ci sono in previsione
-- questi verranno azzerati .. se non presenti in variazioni o impegni verranno invalidate
-- quindi possono esserce se abbiamo capitoli o componenti che ci sono in UG e non in UP
-- prima dell'approvazione

-- 100899 invalidati
-- 100399, 100396,100966 azzerato
...
-- Q9 - estrazione capitoli con STA a zero - questi potrebbero tutti rimanere senza componente
select anno.anno_bilancio, tipo.elem_tipo_code,e.elem_code::integer,
