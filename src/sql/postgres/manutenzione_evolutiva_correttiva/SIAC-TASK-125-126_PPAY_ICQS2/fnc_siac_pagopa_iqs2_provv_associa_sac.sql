...
drop FUNCTION if exists siac.fnc_siac_pagopa_iqs2_provvissori_associa_sac
(
...
CREATE OR REPLACE FUNCTION siac.fnc_siac_pagopa_iqs2_provvissori_associa_sac
(
  codiceAssociazione varchar,
  enteProprietarioId integer,
  loginOperazione varchar, 
...
DECLARE
   ...
   nomeTabella varchar:='pagopa_t_iqs2_elabora_provv_associa_sac';
...
BEGIN
	...
	strMessaggioFinale:='Provvisori di cassa IQS2 - associazione SAC.';
    strMessaggioLog:='Inizio fnc_siac_pagopa_iqs2_provvissori_associa_sac - '||strMessaggioFinale;
    ...
    strMessaggioLog:='Continua fnc_siac_pagopa_iqs2_provvissori_associa_sac - '||strMessaggioFinale;
    strMessaggio:='Creazione tabella pagopa_t_iqs2_elabora_provv_associa_sac.';
    strMessaggioLog:=strMessaggioLog||strMessaggio;
    raise notice '%',strMessaggioLog;
    create temporary table pagopa_t_iqs2_elabora_provv_associa_sac
    as select distinct ric.pagopa_iqs2_ric_anno_eserc as provc_anno, ric.pagopa_iqs2_ric_num_prov as provc_numero, ric.pagopa_iqs2_ric_associa_sac  as associa_sac_code
    from siac_t_file_pagopa_iqs2 pagopa, siac_d_file_pagopa_iqs2_stato stato,pagopa_t_iqs2_riconciliazione  ric,pagopa_r_iqs2_configura_sac  r 
    where stato.ente_proprietario_id=enteProprietarioId
    and   stato.file_pagopa_iqs2_stato_code in (ACQUISITO_ST,ELABORATO_IN_CORSO_ST,ELABORATO_IN_CORSO_SC_ST,ELABORATO_IN_CORSO_ER_ST)
    and   pagopa.file_pagopa_iqs2_stato_id=stato.file_pagopa_iqs2_stato_id
    and   pagopa.file_pagopa_iqs2_anno in (annoBilancio_ini,annoBilancio)
    and   ric.file_pagopa_iqs2_id =pagopa.file_pagopa_iqs2_id 
    and   ric.pagopa_iqs2_ric_flusso_stato_elab ='N'
    and   r.ente_proprietario_id =ric.ente_proprietario_id 
    and   r.pagopa_iqs2_conf_sac_code =ric.pagopa_iqs2_ric_associa_sac 
    and   ric.pagopa_iqs2_ric_associa_sac = coalesce(codiceAssociazione,ric.pagopa_iqs2_ric_associa_sac)
   ...
   and    r.data_cancellazione is null
    ...
    from fnc_siac_provvisorio_associa_sac
     (
      ...
      nomeTabella,
      0,
      coalesce(codiceAssociazione,null),
      loginOperazione, 
      dataElaborazione
     );
   ...
   strMessaggioLog:='Fine fnc_siac_pagopa_iqs2_provvissori_associa_sac - '||strMessaggioFinale;
...
alter function
siac.fnc_siac_pagopa_iqs2_provvissori_associa_sac
(
  varchar, 
  integer,
  varchar,
