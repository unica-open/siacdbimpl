...
drop table if exists siac.siac_t_tefa_trib_importi;
...
tefa_trib_upload_id       integer  NOT null, -- dovrebbero essere del FK
tefa_file_id              integer NOT null,  -- dovrebbero essere del FK
...
COMMENT ON COLUMN siac.siac_t_tefa_trib_importi.tefa_trib_upload_id
IS 'Upload ID a raggruppamento caricamento n-files';

COMMENT ON COLUMN siac.siac_t_tefa_trib_importi.tefa_file_id
IS 'Identificativo file';
...
CREATE INDEX siac_t_tefa_trib_fk_upload_id_idx ON siac.siac_t_tefa_trib_importi
  USING btree (tefa_trib_upload_id);

CREATE INDEX siac_t_tefa_trib_fk_tefa_file_id_idx ON siac.siac_t_tefa_trib_importi
  USING btree (tefa_file_id);
...
tefa_trib_tipologia_id integer  null,
validita_inizio  TIMESTAMP WITHOUT TIME ZONE  NOT NULL default now(),
validita_fine    TIMESTAMP WITHOUT TIME ZONE,
data_creazione   TIMESTAMP WITHOUT TIME ZONE NOT NULL default now(),
data_modifica    TIMESTAMP WITHOUT TIME ZONE,
...
tefa_trib_upload_id integer, -- FK
