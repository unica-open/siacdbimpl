...
select
   ...
   (case when tefa.tefa_trib_anno_rif::integer<='||annoPrecPrec::VARCHAR||' then '||annoPrecPrec::VARCHAR||
   	     ' when tefa.tefa_trib_anno_rif::integer='||annoPrec::varchar||' then tefa.tefa_trib_anno_rif::integer
           when tefa.tefa_trib_anno_rif::integer>='||p_tefa_trib_comune_anno::varchar||' then tefa.tefa_trib_anno_rif::integer else null end ) anno_di_riferimento,
...
from siac_t_tefa_trib_importi tefa left join siac_d_tefa_trib_comune com on (com.ente_proprietario_id=tefa.ente_proprietario_id and com.tefa_trib_comune_code=tefa.tefa_trib_comune_code and com.data_cancellazione is null ),
...
' and   trib.tefa_trib_code=tefa.tefa_trib_tributo_code
  ...
  group by tefa.tefa_trib_file_id,
         ...
         (case when tefa.tefa_trib_anno_rif::integer<='||annoPrecPrec::VARCHAR||' then '||annoPrecPrec::VARCHAR||
   	     ' when tefa.tefa_trib_anno_rif::integer='||annoPrec::varchar||' then tefa.tefa_trib_anno_rif::integer
           when tefa.tefa_trib_anno_rif::integer>='||p_tefa_trib_comune_anno::varchar||' then tefa.tefa_trib_anno_rif::integer else null end ),
