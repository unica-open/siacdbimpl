...
*/
-- 16.01.2019 Sofia HD-INC000002927808_Arpea

-- HD-INC000002927808_Arpea_ConfAvvioSiope+_16012019.sql
...
where e.ente_proprietario_id=10;

-- richiedere i valori seguenti
update siac_t_ente_oil e
set    ente_oil_codice_ipa='UFXE5M', -- <codice_ente>
       ente_oil_codice_istat='24760049', -- <codice_istat_ente>
       ente_oil_codice_tramite='A2A-04255500', -- <codice_tramite_ente>
       ente_oil_codice_tramite_bt='A2A-38628908', -- <codice_tramite_BT>
       ente_oil_codice_pcc_uff='arpea_to',
...
where ente.ente_proprietario_id=10
and not exists
(
...
where plus.ente_proprietario_id=10
...
set    accredito_tipo_id=accreSTI.accredito_tipo_id,
       data_modifica=now(),
...
from siac_D_accredito_tipo tipo,siac_d_accredito_gruppo gruppo,
     ...
     siac_d_accredito_tipo accreSTI
where tipo.ente_proprietario_id=10
...
and   tipo.accredito_gruppo_id=gruppo.accredito_gruppo_id
and   accreSTI.ente_proprietario_id=tipo.ente_proprietario_id
and   accreSTI.accredito_tipo_code='STI'
...
/*Modalità di accredito		Soggetto		Ordine
ST	STIPENDI - A COPERTURA	177	COMMISSARIO STRAORDINARIO	2
ST	STIPENDI - A COPERTURA	3	DIRETTORE ENTE	2
ST	STIPENDI - A COPERTURA	353	COMMISSARIO STRAORDINARIO BIS	2
ST	STIPENDI - A COPERTURA	4	DIPENDENTI ENTE	2*/
...
and   oil.accredito_tipo_oil_code='05'
and   tipo.ente_proprietario_id=oil.ente_proprietario_id
and   tipo.accredito_tipo_code in ('CCP')
and   tipo.data_cancellazione is null
and   not exists
(
select 1
...
and   oil.accredito_tipo_oil_code='07'
and   tipo.ente_proprietario_id=oil.ente_proprietario_id
and   tipo.accredito_tipo_code in ('AS','ASC')
and   tipo.data_cancellazione is null
and   not exists
(
select 1
...
and   oil.accredito_tipo_oil_code='08'
and   tipo.ente_proprietario_id=oil.ente_proprietario_id
and   tipo.accredito_tipo_code in ('F2','F24EP')
and   tipo.data_cancellazione is null
and   not exists
(
select 1
...
and   oil.accredito_tipo_oil_code='17'
and   tipo.ente_proprietario_id=oil.ente_proprietario_id
and   tipo.accredito_tipo_code in ('STI','AP','TT','DEE')
and   tipo.data_cancellazione is null
and   not exists
(
select 1
...
set     data_cancellazione=now()
where r.ente_proprietario_id=10
