...
*/
﻿-- 08.01.2019 Sofia HD-INC000002927959_p102
-- HD-INC000002927959_p102_ConfAvvioSiope+_08012019.sql
...
set    flusso_elab_mif_xml_out=true,
       flusso_elab_mif_attivo=true
where mif.ente_proprietario_id=14
and   exists
(
select 1
...
set     data_cancellazione=now(),
        validita_fine=now(),
        login_operazione=rs.login_operazione||'INC000002927959-admin-siope+'
from siac_t_modpag mdp, siac_D_accredito_tipo tipo,siac_d_accredito_gruppo gruppo,
...
where tipo.ente_proprietario_id=14
and   tipo.accredito_tipo_code in ('GF','GFB','F2','CBI')
...
select
fnc_mif_flusso_elaborato_firme
( 14,
  2019,
  'P102',
  'RICFIMIF',
...
select oil.oil_ricevuta_errore_id,
       er.oil_ricevuta_errore_code,
       er.oil_ricevuta_errore_desc,
       oil.*
from siac_t_oil_ricevuta oil,siac_d_oil_ricevuta_errore er
where oil.flusso_elab_mif_id=58896
and   oil.data_cancellazione is null
and   oil.oil_ricevuta_errore_id=er.oil_ricevuta_errore_id
...
select distinct e.ente_proprietario_id
from siac_d_oil_ricevuta_tipo e
where e.ente_proprietario_id in (4,5,10,13,14,16,29)
and   e.oil_ricevuta_tipo_code ='F'

select distinct e.ente_proprietario_id
from siac_d_oil_esito_derivato e,siac_d_oil_ricevuta_tipo tipo
where e.ente_proprietario_id in (4,5,10,13,14,16,29)
and   tipo.oil_ricevuta_tipo_id=e.oil_ricevuta_tipo_id
and  tipo.oil_ricevuta_tipo_code='F'

select *
from siac_d_oil_qualificatore e
...
select '00',
       ...
       tipo.ente_proprietario_id
from siac_d_oil_ricevuta_tipo tipo
where tipo.ente_proprietario_id in (14,29)
and   tipo.oil_ricevuta_tipo_code='F'
and   not exists
(
