...
*/
CREATE OR REPLACE VIEW siac.siac_v_dwh_sac (
    cod_sac,
    desc_sac,
    tipo,
    id,
    ...
    validita_fine)
AS
SELECT t1.classif_code AS cod_sac, t1.classif_desc AS desc_sac,
...
FROM ( WITH RECURSIVE rqname(classif_classif_fam_tree_id, classif_fam_tree_id,
    ...
    WHERE cf.classif_fam_id = tt1.classif_fam_id AND tt1.classif_fam_tree_id =
        rt1.classif_fam_tree_id AND rt1.classif_id_padre IS NULL AND cf.classif_fam_desc::text = 'Struttura Amministrativa Contabile'::text AND tt1.ente_proprietario_id = rt1.ente_proprietario_id
    UNION ALL
