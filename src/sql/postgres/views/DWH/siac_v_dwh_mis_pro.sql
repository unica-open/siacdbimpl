...
*/
CREATE OR REPLACE VIEW siac.siac_v_dwh_mis_pro (
    cod_missione_programma,
    desc_missione_programma,
    tipo,
    id,
    id_padre,
    ...
    validita_fine)
AS
SELECT t1.classif_code AS cod_missione_programma,
    t1.classif_desc AS desc_missione_programma, ti1.classif_tipo_desc AS tipo,
    tb.classif_id AS id, tb.classif_id_padre AS id_padre,
    tb.ente_proprietario_id, tb.level AS livello, tb.validita_inizio,
    tb.validita_fine
FROM ( WITH RECURSIVE rqname(classif_classif_fam_tree_id, classif_fam_tree_id,
    ...
    WHERE cf.classif_fam_id = tt1.classif_fam_id AND tt1.classif_fam_tree_id =
        rt1.classif_fam_tree_id AND rt1.classif_id_padre IS NULL AND cf.classif_fam_desc::text = 'Spesa - MissioniProgrammi'::text AND tt1.ente_proprietario_id = rt1.ente_proprietario_id
    UNION ALL
    ...
    SELECT rqname.classif_classif_fam_tree_id, rqname.classif_fam_tree_id,
            ...
            rqname.ente_proprietario_id, rqname.ordine, rqname.validita_inizio,
            rqname.validita_fine, rqname.livello, rqname.level
    FROM rqname
