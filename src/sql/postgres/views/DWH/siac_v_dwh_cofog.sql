...
*/
CREATE OR REPLACE VIEW siac.siac_v_dwh_cofog (
    cod_cofog,
    desc_cofog,
    tipo,
    id,
    ...
    validita_fine)
AS
SELECT t1.classif_code AS cod_cofog, t1.classif_desc AS desc_cofog,
...
FROM ( WITH RECURSIVE rqname(classif_classif_fam_tree_id, classif_fam_tree_id,
    ...
    WHERE cf.classif_fam_id = tt1.classif_fam_id AND tt1.classif_fam_tree_id =
        rt1.classif_fam_tree_id AND rt1.classif_id_padre IS NULL AND cf.classif_fam_desc::text = 'Cofog - Classificazione internazionale della spesa pubblica per funzione'::text AND tt1.ente_proprietario_id = rt1.ente_proprietario_id
    UNION ALL
