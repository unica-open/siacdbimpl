...
*/
﻿CREATE VIEW siac.siac_v_bko_bil_elem_ente_classif_e (
...
FROM (
    ...
    WHERE be.bil_id = b.bil_id AND tp.periodo_id = b.periodo_id AND
        et.elem_tipo_id = be.elem_tipo_id AND ep.ente_proprietario_id = b.ente_proprietario_id AND date_trunc('day'::text, now()) > b.validita_inizio AND (date_trunc('day'::text, now()) < b.validita_fine OR b.validita_fine IS NULL) AND date_trunc('day'::text, now()) > be.validita_inizio AND (date_trunc('day'::text, now()) < be.validita_fine OR be.validita_fine IS NULL) AND date_trunc('day'::text, now()) > et.validita_inizio AND (date_trunc('day'::text, now()) < et.validita_fine OR et.validita_fine IS NULL) AND (et.elem_tipo_id = ANY (ARRAY[1, 2, 3])) AND cla.elem_id = be.elem_id AND tcl.classif_id = cla.classif_id AND tic.classif_tipo_id = tcl.classif_tipo_id AND date_trunc('day'::text, now()) > cla.validita_inizio AND (date_trunc('day'::text, now()) < cla.validita_fine OR cla.validita_fine IS NULL) AND date_trunc('day'::text, now()) > tcl.validita_inizio AND (date_trunc('day'::text, now()) < tcl.validita_fine OR tcl.validita_fine IS NULL) AND date_trunc('day'::text, now()) > tic.validita_inizio AND (date_trunc('day'::text, now()) < tic.validita_fine OR tic.validita_fine IS NULL)
    ORDER BY be.ordine
    ) tb;