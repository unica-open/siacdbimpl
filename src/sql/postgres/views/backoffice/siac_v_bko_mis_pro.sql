...
*/
﻿CREATE OR REPLACE VIEW siac.siac_v_bko_mis_pro(
...
AS
  ...
  AND
                             ...
                             cf.classif_fam_code='00001' AND
                             tt1.ente_proprietario_id = rt1.ente_proprietario_id
  AND
    date_trunc('day'::text, now()) BETWEEN rt1.validita_inizio and coalesce (rt1.validita_fine,  date_trunc('day'::text, now()))
                       UNION ALL
                       ...
                       WHERE
                           date_trunc('day'::text, now()) BETWEEN tn.validita_inizio and coalesce (tn.validita_fine,  date_trunc('day'::text, now())) 
                       and tp.classif_id = tn.classif_id_padre AND
