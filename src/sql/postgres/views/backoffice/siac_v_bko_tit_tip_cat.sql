...
*/
﻿CREATE or replace VIEW siac.siac_v_bko_tit_tip_cat (
...
rt1.classif_fam_tree_id AND rt1.classif_id_padre IS NULL AND 
cf.classif_fam_code='00003'
AND tt1.ente_proprietario_id = rt1.ente_proprietario_id 
AND date_trunc('day'::text, now()) 
between rt1.validita_inizio and coalesce(rt1.validita_fine, date_trunc('day'::text, now()))  UNION ALL
...
between tn.validita_inizio and coalesce(tn.validita_fine, date_trunc('day'::text, now()))
and 
    tp.classif_id = tn.classif_id_padre AND tn.ente_proprietario_id =
