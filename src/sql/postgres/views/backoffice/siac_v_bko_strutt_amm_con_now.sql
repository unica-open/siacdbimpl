...
*/
CREATE OR REPLACE VIEW siac.siac_v_bko_strutt_amm_con_now(
...
AS
  ...
  FROM (
         ...
         FROM (WITH RECURSIVE rqname(classif_classif_fam_tree_id,
           classif_fam_tree_id, classif_id, classif_id_padre,
           ente_proprietario_id, ordine, livello, validita_inizio,
           validita_fine, level, arrhierarchy) AS (
                                                    SELECT
  ...
  ,
                                                    ...
                                                    FROM siac_r_class_fam_tree
                                                      rt1,
                                                         siac_t_class_fam_tree
                                                           tt1,
                                                         siac_d_class_fam cf
                                                    WHERE cf.classif_fam_id =
                                                      tt1.classif_fam_id AND
  ...
  WHERE now() >= tb.validita_inizio AND
        now() <= COALESCE(tb.validita_fine::timestamp with time zone, now())
