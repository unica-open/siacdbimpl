...
*/
﻿CREATE OR REPLACE VIEW siac.siac_v_bko_tit_mac(
...
AS
  ...
  AND
                             ...
                             cf.classif_fam_code::text = '00002'::text AND
                             tt1.ente_proprietario_id = rt1.ente_proprietario_id
  AND
                             date_trunc('day'::text, now()) >=
                               rt1.validita_inizio AND
                             date_trunc('day'::text, now()) <= COALESCE(
                               rt1.validita_fine::timestamp with time zone,
                               date_trunc('day'::text, now()))
                       UNION ALL
                       ...
                       WHERE date_trunc('day'::text, now()) >=
                         tn.validita_inizio AND
                             date_trunc('day'::text, now()) <= COALESCE(
                               tn.validita_fine::timestamp with time zone,
                               date_trunc('day'::text, now())) AND
                             tp.classif_id = tn.classif_id_padre AND
