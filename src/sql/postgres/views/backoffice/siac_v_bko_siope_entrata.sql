...
*/
﻿CREATE OR REPLACE VIEW siac.siac_v_bko_siope_entrata(
...
AS
  ...
  AND
                             ...
                             cf.classif_fam_desc::text = 'Siope - Entrata'::text
  AND
  ...
  AND
                             date_trunc('day'::text, now()) > tn.validita_inizio
  AND
                             (date_trunc('day'::text, now()) < tn.validita_fine
  OR
                             tn.validita_fine IS NULL)
       )
